...
public class DisabledPanel extends JPanel
{
	...
	/**
	 *  Convenience static method to disable or enable all components of a given
	 *  Container, including nested Containers.  DOES NOT TRACK like enable/disable methods.
	 *
	 *  @param container the Container containing Components to be disabled
	 */
	public static void setEnabledRecursive(Container container, boolean isEnabled, JComponent[] componentsToIgnore) {
		List<JComponent> components =
			SwingUtils.getDescendantsOfType(JComponent.class, container, true);
		
		for (JComponent c : componentsToIgnore) {
			boolean success = components.remove(c);
//			System.out.println("setEnabledRecursive: ignoring a component and successfully removed from list: "+success);
		}
