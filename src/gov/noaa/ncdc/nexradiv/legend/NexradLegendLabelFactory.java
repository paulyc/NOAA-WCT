...
//                String icao = nexradFile.substring(x + 0, x + 4).toUpperCase();
                ...
                //   fileDate.substring(0, 4) + " " + fileHour + ":" + fileMin + ":"+ fileSec +" Z");
...
//              header.getMinuteString() + ":"+ header.getSecondString() +" Z");
                
                legend.setDateTimeInfo(new String[] { fileDate.substring(4, 6) + "/" + fileDate.substring(6, 8) + "/" +
                        fileDate.substring(0, 4) + " " + fileHour + ":" + fileMin + ":"+ fileSec +" Z" } );
                ...
                try {
                    ...
                    legend.setDateTimeInfo(new String[] { nexradFile.substring(5, 7) + "/" + nexradFile.substring(7, 9) + "/" +
                            nexradFile.substring(9, 13) + " " + nexradFile.substring(14, 16) + ":00:00 Z" });
...
//                }
                ...
                //metaLabel[2].setText(fileDate.substring(4, 6) + "/" + fileDate.substring(6, 8) + "/" +
                //   fileDate.substring(0, 4) + " " + fileHour + ":" + fileMin +" Z");
//                String dateString = new Integer(header.getDate()).toString();
//                String dateString.substring(4, 6) + "/" + dateString.substring(6, 8) + "/" +
//                        dateString.substring(0, 4) + " " + header.getHourString() + ":" + 
//                        header.getMinuteString() + ":"+ header.getSecondString() +" Z" });

                String prodDateString = WCTUtils.LEGEND_FORMATTER.format(new Date(((DecodeL3Header)header).getMilliseconds()));
                String scanDateString = WCTUtils.LEGEND_FORMATTER.format(new Date(((DecodeL3Header)header).getScanMilliseconds()));
                legend.setDateTimeInfo(new String[] {
                		prodDateString+" Z",
                		scanDateString+" Z (VOL)"
...
//            int sweepNum = Integer.parseInt(decoder.getDecodeHints().get("startSweep").toString().trim());
            if (raster.getLastDecodedCappiHeightInMeters() == null ||
            	...
            	if (raster.getLastDecodedSweepTime() != null) {
            		...
            		sdfHHMMSS.setTimeZone(TimeZone.getTimeZone("Z"));
            		System.out.println(raster.getLastDecodedSweepTime());
            		l2Metadata[2] = "SWEEP TIME: "+sdfHHMMSS.format(raster.getLastDecodedSweepTime())+" Z";
            	}
            }
            else {
...
//            l2Metadata[2] = "SWEEP TIME: "+dateFormat.format(decoder.getSweepDateTime()[0])+" Z";
...
//                    specialMetadata[1] = "MAX: " + ((DecodeL3Header)header).getProductSpecificValue(3) + " DBZ";
                    ...
                    specialMetadata[1] = "BASE VEL. 248 NM";
                }
            }
...
//                    specialMetadata[0] = "MAX: " + fmt2.format(((DecodeL3Header)header).getProductSpecificValue(3)*hw32*0.01) + " IN";
                    ...
                    specialMetadata[1] = "BEG: " + NexradEquations.convertJulianDateFormatted(((DecodeL3Header)header).getProductSpecificValue(0)) +
                        ...
                        ":" + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(1) % 60) + " Z";
                    specialMetadata[2] = "END: " + NexradEquations.convertJulianDateFormatted(((DecodeL3Header)header).getProductSpecificValue(4)) +
                        ...
                        ":" + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(5) % 60) + " Z";
            ...
            // STORM TOTAL PRECIPITATION
                ...
                if (!isAlphaBackground) {
                    ...
                    specialMetadata[2] = "END: " + NexradEquations.convertJulianDateFormatted(((DecodeL3Header)header).getProductSpecificValue(6)) +
                            " " + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(7) / 60) +
                            ":" + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(7) % 60) + " Z";
                }
                else {
...
//                    specialMetadata[0] = "MAX: " + ((DecodeL3Header)header).getProductSpecificValue(3) + " KG/M2";
                ...
                if (!isAlphaBackground) {
                    ...
                    specialMetadata[0] = "MAX: " + 
            		fmt2.format((double) ((DecodeL3Header)header).getProductSpecificValue(3) / 10.0) + " IN";
                    specialMetadata[1] = "TIME SPAN: " + 
                    	fmt0.format(((DecodeL3Header)header).getProductSpecificValue(1) / 60) +
                    	":" + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(1) % 60);
                    specialMetadata[2] = "END: " +
                    		NexradEquations.convertJulianDateFormatted(((DecodeL3Header)header).getProductSpecificValue(4)) + " " +
                    	fmt0.format(((DecodeL3Header)header).getProductSpecificValue(0) / 60) +
                        ":" + fmt00.format(((DecodeL3Header)header).getProductSpecificValue(0) % 60)+ " Z";
...
//                    fmt2.format((double) us / 100.0) + " IN/HR";
            ...
            else if (pcode == NexradHeader.L3PC_MET_SIGNAL) {
                if (!isAlphaBackground) {
                    dataDescription[0] = "MET SIGNAL";
                    specialMetadata[0] = " ";
                    specialMetadata[1] = " ";
                    specialMetadata[2] = " ";
                }
