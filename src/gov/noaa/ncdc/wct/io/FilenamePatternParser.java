...
//		System.out.println(date);
    ...
    /**
     * Gets the default color table.  If a product code and lookup table is used, attempt to get from the 
     * filename, otherwise look for the 'defaultColorTable' element in the filename pattern element.
     * @param fp
     * @param filename
     * @return
     * @throws IOException 
     */
    public static String getDefaultColorTable(FilenamePattern fp, String filename) throws IOException {
        StringBuilder productString = new StringBuilder();
        
        // if it is specified in the filename pattern xml, that takes precedence over per product
        if (fp.getDefaultColorTable() != null) {
        	return fp.getDefaultColorTable();
        }

        // per product is optional
        if (fp.getProductCode() == null) {
            return null;
        }
        ...
        String colorTable = ProductLookupTableManager.getInstance().getProductDefaultColorMap(productString.toString());
...
//				System.out.println("FOUND DATA TYPE MATCH!!!");
	...
	public static String getCollectionReference(FilenamePattern p, String filename) throws MalformedURLException {
		return p.getCollectionReference();
	}
