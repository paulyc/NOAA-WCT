...
public class ColorsAndValues {
    ...
    public ColorsAndValues(Color[] colors, Double[] values, String units) {
        this.colors = colors;
        this.values = values;
    ...
    public ColorsAndValues(Color[] colors, Double[] values, String[] labels, String units) {
        this.colors = colors;
        this.values = values;
        this.labels = labels;
        this.units = units;
    ...
    public String[] getLabels(DecimalFormat fmt) {
    	...
    	if (values == null || (values.length == 1 && values[0].isNaN())) {
...
//            }
    ...
    /**
     * Creates 144 unique equally spaced colors and values by interpolating
     * @param colorsAndVals
     * @return
     * @throws WCTException
     */
...
//            }
        ...
        if (! everFoundValue) {
            System.out.println("no color map ever matched: "+colorsAndVals);
            return new ColorsAndValues(new Color[] { Color.BLACK }, new Double[] { Double.NaN }, "");
        }
...
//      System.out.println("end color/val calc");
        ...
        return new ColorsAndValues(equalColors, equalVals, colorsAndVals.getUnits());
