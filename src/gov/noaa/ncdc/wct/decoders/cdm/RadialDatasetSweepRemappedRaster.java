...
public class RadialDatasetSweepRemappedRaster implements WCTRaster {
	...
	private RadialDatasetSweepAdapter radialDataset = null;
	...
	public synchronized void process(String source, java.awt.geom.Rectangle2D.Double bounds) 
		...
		try {
			...
			if (isNewFile) {
				...
				radialDataset = (RadialDatasetSweepAdapter) FeatureDatasetFactoryManager.open(
						ucar.nc2.constants.FeatureType.RADIAL, 
						source, cancelTask, errlog);
				
//				radialDataset.close();
//
//				FeatureDataset fd = FeatureDatasetFactoryManager.open(null, source, WCTUtils.getSharedCancelTask(), errlog);
//				if (fd.getFeatureType() == FeatureType.RADIAL || fd.getFeatureType() == FeatureType.STATION_RADIAL) {
////					radialDataset = (RadialDatasetSweepAdapter)fd;
//					System.out.println("FOUND RADIAL");
//				}
//				fd.close();
...
//			Variable v = radialDataset.getNetcdfFile().findVariable(variableName);
			...
			try {
				// set the sweep time as halfway between starting and ending azimuths/rays 
				this.lastSweepTime = radialDataset.getTimeUnits().makeDate(sweep.getTime(sweep.getAzimuth().length/2));				
			} catch (Exception e) {
				System.err.println("ERROR GETTING LAST SWEEP TIME: ");
				e.printStackTrace();
			} 
...
//			MathTransform nexradTransform = nexradProjection.getRadarTransform(siteLon, siteLat).inverse();
			// Changed to use Unidata projection library part of NetCDF-Java
//			FlatEarth feproj = new FlatEarth(siteLat, siteLon);
	        EquidistantAzimuthalProjection ncjProj = new EquidistantAzimuthalProjection(siteLat, siteLon, 0, 0, new Earth(6378137, 6356752.3141, 298.257222101));
...
//			radialDataset.getNetcdfFile().close();
			radialDataset.close();
			...
			for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					for (int c = 0; c < cappiHeightsInMeters.length; c++) {
						this.raster.setSample(i, j, c, Float.NaN);
					}
				}
			}
...
//						srcPoints[1] = lat;
//						nexradTransform.transform(srcPoints, 0, dstPoints, 0, 1);
//						double distanceOld = Math.sqrt((dstPoints[0]*dstPoints[0]) + (dstPoints[1]*dstPoints[1]))/1000;
...
//						System.out.println("OLD: "+dstPoints[0]+","+dstPoints[1]+",  dist="+distanceOld);
//						System.out.println("NEW: "+pp.getX()+","+pp.getY()+",  dist="+distance);
