...
public class GridDatasetRemappedRaster implements WCTRaster {
	...
	public static final int DEFAULT_REMAPPED_GRID_WIDTH = 1800;
	public static final int DEFAULT_REMAPPED_GRID_HEIGHT = 1800;
	...
	private boolean cacheGrid = true;
	...
	private void process(String source, java.awt.geom.Rectangle2D.Double bounds, double smoothingFactor, boolean scanOnly) 
		...
		try {
            ...
            if (gridIndex < 0) {
            	...
            	for (GridDatatype g : gds.getGrids()) {
            		sb.append("'"+g.getName()+"' ");
            	}
            	logger.warning("Grid named '"+gridVariableName+"' was not found in dataset. \n"
            			+ "Defaulting to first grid '"+gds.getGrids().get(0).getName()+"' \n"
            			+ "Available grids are: "+sb.toString());
            	gridIndex = 0;
            }
			...
			// strangely, the Grid objects don't reflect this alternate order, so the base
...
//			System.out.println("X DIM: "+grid.getXDimension() + " index="+rawXIndex+ " or "+grid.getXDimensionIndex());
//			System.out.println("Y DIM: "+grid.getYDimension() + " index="+rawYIndex+ " or "+grid.getYDimensionIndex());
//			NetCDF for Java seems to have fixed this to be smarter about the GridDatatype understanding unconventionally ordered data
...
//			}
				...
				// special workaround for goes-16 - enable experimental row read chunking strategy
//				cacheGrid = ! (source.contains("_G16_s") && source.contains("OR_ABI-"));
			cacheGrid = grid.getYDimension().getLength() < 5000;
...
//			Only create a new raster if needed			
			if (this.raster == null || this.width != raster.getWidth() || this.height != raster.getHeight()) {

				if (width == 0 || height == 0) {
					width = DEFAULT_REMAPPED_GRID_WIDTH;
					height = DEFAULT_REMAPPED_GRID_HEIGHT;
				}
			...
			for (int zy=0; zy<numYTiles; zy++) {
				for (int zx=0; zx<numXTiles; zx++) {
					...
					//--------------------------------------------------------------------
//					System.out.println("checking 2-d coord system");
					...
					// reset min/max values if we are not caching
					if (! cacheGrid) {
...
//					for (int y=height-1; y>=0; y--) {  // faster in reverse? //
...
//			this.gds.close();  //--- DON'T CLOSE - let user close with .close() method in this class
...
//			rowCacheOrder.clear();
	...
	/**
	 * Returns true if we are caching the entire grid.  For huge grids, to save memory 
	 * we split into chunks of rows to temporarily read and cache, instead of the entire grid.
	 * @return
	 */
	public boolean isEntireGridCached() {
		return cacheGrid;
	}
	...
	public void calculateStatistics() throws Exception {
		...
		while (rowsRead < nrows) {
			...
			gridMaxValue = (gridMaxValue < minMax.max) ? minMax.max : gridMaxValue;
			gridMinValue = (gridMinValue > minMax.min) ? minMax.min : gridMinValue;
	 ...
	 * @throws Exception 
	 */
	private double[] getRowDataChunk(int row, int rowChunkSize) throws Exception {
...
//			System.out.println("reading from cache: "+row);						
	...
	private double getCellValue(int x, int y) throws Exception {
		...
		// TODO: hardcode for now, but leave in case we implement low-memory row caching (similar to GoesRemappedRaster)
//		boolean cacheGrid = false;
		double value;
...
//			value = getRowData(y)[x];						
			...
			int rowChunkIndex = y / ROW_CHUNK_SIZE;
			int minrow = rowChunkIndex * ROW_CHUNK_SIZE;
			int maxrow = ((rowChunkIndex+1) * ROW_CHUNK_SIZE) - 1;
			maxrow = (maxrow > imgHeight) ? imgHeight-1 : maxrow;
			
			int valueIndex = (y-minrow)*imgWidth + x;
					
			value = getRowDataChunk(y, ROW_CHUNK_SIZE)[valueIndex];
			
			gridMaxValue = (gridMaxValue < value) ? value : gridMaxValue;
			gridMinValue = (gridMinValue > value) ? value : gridMinValue;
		}
	...
	/**
	 * Closes the underlying GridDataset if it is not null
	 * @throws IOException 
	 */
