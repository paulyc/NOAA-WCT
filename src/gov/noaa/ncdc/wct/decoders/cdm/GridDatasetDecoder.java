...
import gov.noaa.ncdc.wct.export.WCTExportGridNotFoundException;
...
public class GridDatasetDecoder implements StreamingDecoder {
    ...
    private GridDatasetRowReader gridDatasetRowReader = new GridDatasetRowReader();
...
//        schema = FeatureTypeFactory.newFeatureType(POINT_ATTRIBUTES, "Grid Dataset Cell Centroid Point Attributes");
    ...
    public void decodeData(StreamingProcess[] streamingProcessArray) throws DecodeException {
        ...
        try {
            ...
            gridDatasetRowReader.clearAndResetCache();
            ...
            if (filter.getExtentFilter() != null) {
                ...
                System.out.println(this.grid.getCoordinateSystem().getLatLonBoundingBox().toString2());
            
                // don't even try to subset if invalid bounding box
                if (! this.grid.getCoordinateSystem().getLatLonBoundingBox().equals(LatLonRect.INVALID)) {
                	System.out.println("Invalid bounding box");
                	try {
                		GridDatatype tmpgrid = grid.makeSubset(null, null, bbox, 1, 1, 1);
                		this.grid = tmpgrid;
                	} catch (Exception e) {
                		System.out.println("Valid bounding box, but caught exception while trying to subset.");
                		e.printStackTrace();
                	}
                }
...
//	        System.out.println("dataCache: "+Arrays.toString(dataCache.getShape())+"  get cell value: "+x+","+y);
...
//			value = getRowData(y)[x];						
			...
			int rowChunkIndex = y / GridDatasetRowReader.ROW_CHUNK_SIZE;
			int minrow = rowChunkIndex * GridDatasetRowReader.ROW_CHUNK_SIZE;
			int maxrow = ((rowChunkIndex+1) * GridDatasetRowReader.ROW_CHUNK_SIZE) - 1;
			maxrow = (maxrow > imgHeight) ? imgHeight-1 : maxrow;
			
			int valueIndex = (y-minrow)*imgWidth + x;
					
			value = gridDatasetRowReader.getRowDataChunk(grid, timeIndex, runtimeIndex, zIndex, 
					y, GridDatasetRowReader.ROW_CHUNK_SIZE)[valueIndex];
    ...
    private void populateDataCache() throws InvalidRangeException, IOException {
        ...
        System.out.println("rt="+runtimeIndex+"  timeIndex="+timeIndex+"  zIndex="+zIndex);
...
//        System.out.println(runtimeIndex + " "+timeIndex+" "+zIndex);
   ...
   public Date getLastProcessedDateTime() {
       ...
       try {
           return this.coordSys.getTimeAxis1D().getCalendarDate(this.timeIndex).toDate();
       } catch (Exception e) {
           return null;
       }
   }
   ...
   public Date getLastProcessedRuntime() {
       ...
       try {
           return this.coordSys.getRunTimeAxis().getCalendarDate(this.runtimeIndex).toDate();
       } catch (Exception e) {
           return null;
       }
   }
