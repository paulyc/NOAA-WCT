...
public class GridDatasetUtils {
	...
	private final static HashMap<String, GridDataset> gdCache = new HashMap<String, GridDataset>();
	private final static ArrayList<GridDataset> gdScheduledToCloseList = new ArrayList<GridDataset>();
...
//        NetcdfDataset.initNetcdfFileCache(0, 2, 2, 60*10);
        ...
        // process and close all gds objects that have been scheduled to close
        for (GridDataset g : gdScheduledToCloseList) {
        	g.close();
        }
	...
	public static void closeCached() throws IOException {
		for (GridDataset g : gdCache.values()) {
			g.close();
			g.release();
		}
	}
	...
	public static void scheduleToClose(GridDataset gds) {
		...
		gdScheduledToCloseList.add(gds);
	}
