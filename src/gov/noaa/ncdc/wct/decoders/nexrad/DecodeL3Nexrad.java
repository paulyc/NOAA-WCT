...
/*    
  Steps required to add new Level-3 product:
...
//        	return (level == 0 || level == 128) ? Double.NaN : (level-128)*0.01;
        ...
        else if (header.getProductCode() == NexradHeader.L3PC_MET_SIGNAL) {
//    		System.out.println(scaleOffset[0] + " , "+ scaleOffset[1]);
//        	return (level == 0 || level == 128) ? Double.NaN : (level-128)*0.01;
        	return (level <= 29) ? Double.NaN : (((float)level-scaleOffset[1])/scaleOffset[0]);

 //        	value colorcode legend
//        	29    0  < SNR Th
//        	30    1  Unknown
//        	40   70  Flagged
//        	50    4  0
...
//        			;
//        		}
//        		for (int n=0; n<hwTest.length; n++) {
//        			hwTest[n] = f.readShort();
//        			System.out.println("hwTest["+n+"]: "+hwTest[n]);
//        		}
//        		System.out.println();
//        	} catch (Exception e) {
//        		e.printStackTrace();
...
//        inRaf.order(ucar.unidata.io.RandomAccessFile.LITTLE_ENDIAN);
        ...
        if (header.getProductCode() == NexradHeader.L3PC_TDWR_LONG_RANGE_BASE_REFLECTIVITY_8BIT) {
            ...
            binSpacing = 150;
        		...
        		header.getProductCode() == NexradHeader.L3PC_MET_SIGNAL) {
            binSpacing = 250;
        }
        else if (header.getProductCode() == NexradHeader.L3PC_ENHANCED_ECHO_TOPS ||
        	...
        	// hard code per email exchange with Dan Berkowitz (NWS/ROC) on 7/3/2018
            binSpacing = 2000;
        }
//        else if (header.get)

//                System.out.println("BIN SPACING: "+binSpacing+ "dataHeader[4]="+dataHeader[4]);
...
//        }
        ...
        // ADVANCE TO BEGINNING OF PRODUCT SYMBOLOGY BLOCK (BLOCK DIVIDER)
        short hword = 0;
        while ((hword = f.readShort()) != -1) {
            System.out.println(f.getFilePointer()+": "+hword);
        }
