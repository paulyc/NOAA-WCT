...
public class RadarHashtables {
    ...
    public enum MaxRangeUnits { METERS, DECI_DEGREES };
...
//          nexhashState.put("NOP3", nexhashLat.get("KTLX"));
     ...
     * @param searchFilter filter for certain types of sites
     * @return
     */
    public String getClosestICAO(double lat, double lon, double maxRange, SearchFilter searchFilter) {
    	return getClosestICAO(lat, lon, maxRange, searchFilter, null);
    }
     ...
     * @param maxRange (in decimal degrees)
     * @param searchFilter filter for certain types of sites
     * @param sitesToSearch search only the specified sites; overrides searchFilter.  
     * @return
     */
    public String getClosestICAO(double lat, double lon, double maxRange, SearchFilter searchFilter, String[] sitesToSearch) {
    	String[] vals = getClosestICAOWithDistance(lat, lon, maxRange, MaxRangeUnits.DECI_DEGREES, searchFilter, sitesToSearch);
     ...
     * @return
     */
    public String[] getClosestICAOWithDistance(double lat, double lon, 
    		double maxRange, MaxRangeUnits maxRangeUnits,
    		SearchFilter searchFilter, String[] sitesToSearch) {

        double matchingDist = 9999999;
        String matchingIcao = null;
        
        double maxRangeDegFilter = maxRange/112000;
        ...
        while (keyEnum.hasMoreElements()) {
            ...
            // filter on the degrees where the distance calc is fast, before
            // doing the more expensive 'real' distance calc in meters
            if (dist < maxRangeDegFilter) {
            	if (maxRangeUnits == MaxRangeUnits.METERS) {
            		gCalc.setDestinationPoint(keyLon, keyLat);
            		dist = gCalc.getOrthodromicDistance();
            	}
        ...
        if (matchingDist > maxRange) {
        	...
        	return new String[] { matchingIcao, String.valueOf(matchingDist) };
        }
    }
     ...
     * @return
     */
    public String[][] getClosestTwoWithDistance(double lat, double lon, 
    		double maxRange, MaxRangeUnits maxRangeUnits,
    		SearchFilter searchFilter, String[] sitesToSearch) {
        ...
        double matchingDist2 = 9999999;
        String matchingIcao2 = null;
        ...
        while (keyEnum.hasMoreElements()) {
            ...
            if (dist < maxRangeDegFilter) {
            	...
            	if (dist < matchingDist || dist < matchingDist2) {
            		
            		// new 1st closest, move previous 1st to 2nd
            		if (dist < matchingDist) {
            			matchingDist2 = matchingDist;
            			matchingIcao2 = matchingIcao;
            			matchingDist = dist;
            			matchingIcao = key.toString();
            		}
            		else if (dist < matchingDist2) {
                		matchingDist2 = dist;
                		matchingIcao2 = key.toString();
            		}
...
//            		matchingIcao = key.toString();
        ...
        if (matchingDist > maxRange) {
        	...
        	return new String[][] { new String[] { matchingIcao, String.valueOf(matchingDist) },
        							new String[] { matchingIcao2, String.valueOf(matchingDist2) }
        	};
        }
    }
