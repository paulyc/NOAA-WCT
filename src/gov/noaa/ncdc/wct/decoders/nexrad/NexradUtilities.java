...
public class NexradUtilities {
	...
	public static String getUnits(int productCode) {
		...
		try {
			...
			// TEMP - use this to see if .wctpal file has units first, before checking hardcoded stuff below
			units = NexradSampleDimensionFactory.getSampleDimensionAndLabels(productCode).getUnits();
			if (units != null && units.length() > 0) {
				return units;
			}

			System.out.println("NO UNITS found in palette file: IN 'getUnits: productCode="+productCode);
