...
//			}
			...
			if (bounds == null || bounds.equals(GOES_DEFAULT_EXTENT)) {
				...
				// For max extent, adjust bounds to use outer edges of pixels, not centroids.
				ProjectionPointImpl pp2 = new ProjectionPointImpl(1, 1);
				LatLonPointImpl ll2 = new LatLonPointImpl();
				ll2 = (LatLonPointImpl) proj.projToLatLon(pp2, ll2);				
	            double yCellSize = Math.abs(ll2.getLatitude() - ll.getLatitude()); 
	            double xCellSize = Math.abs(ll2.getLongitude() - ll.getLongitude());				 
				// Find min/max lat/lon for each corner, since projections causes inconsistent locations of min/max
				minLat = Math.min(minLat, ll.getLatitude() - yCellSize/2.0);
				minLon = Math.min(minLon, ll.getLongitude() - xCellSize/2.0);
				maxLat = Math.max(maxLat, ll.getLatitude() + yCellSize/2.0);
				maxLon = Math.max(maxLon, ll.getLongitude() + xCellSize/2.0);
				
				pp.setLocation(0, imgHeight-1);
				ll = (LatLonPointImpl) proj.projToLatLon(pp, ll);
				// For max extent, adjust bounds to use outer edges of pixels, not centroids.
				pp2.setLocation(1, imgHeight-2);
				ll2 = (LatLonPointImpl) proj.projToLatLon(pp2, ll2);				
	            yCellSize = Math.abs(ll2.getLatitude() - ll.getLatitude()); 
	            xCellSize = Math.abs(ll2.getLongitude() - ll.getLongitude());
				// Find min/max lat/lon for each corner, since projections causes inconsistent locations of min/max
				minLat = Math.min(minLat, ll.getLatitude() - yCellSize/2.0);
				minLon = Math.min(minLon, ll.getLongitude() - xCellSize/2.0);
				maxLat = Math.max(maxLat, ll.getLatitude() + yCellSize/2.0);
				...
				ll = (LatLonPointImpl) proj.projToLatLon(pp, ll);
				// For max extent, adjust bounds to use outer edges of pixels, not centroids.
				pp2.setLocation(imgWidth-2, imgHeight-2);
				ll2 = (LatLonPointImpl) proj.projToLatLon(pp2, ll2);				
	            yCellSize = Math.abs(ll2.getLatitude() - ll.getLatitude()); 
	            xCellSize = Math.abs(ll2.getLongitude() - ll.getLongitude());
				// Find min/max lat/lon for each corner, since projections causes inconsistent locations of min/max
...
//					int col = (int)projPoint.getX();
					...
					// don't try to look up data values if we are outside of the input grid
...
//					logger.fine("GETTING DATA FOR ("+col+" , "+row+")");
					...
					// NOT NEEDED?  Because already filtered above?
...
//							col < 0 || col >= imgWidth)) {
						...
						//       NOTE: taking greater value simply overwrites no data value, lat/lon to proj point 
						//             conversion ensures that we are only matching centroid of lat/lon pixel
						//             to single overlapping pixel in projection coordinate system and input data grid.
