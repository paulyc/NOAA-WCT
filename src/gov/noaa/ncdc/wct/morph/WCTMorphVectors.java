...
//		String odapURL = "http://nomads.ncdc.noaa.gov/thredds/"
	...
	private static MorphVector getRAP_252_MV(String yyyymmddhh, Rectangle2D.Double extent, Date begDate, Date endDate, Date date,
		...
		String ncssURL = "https://nomads.ncdc.noaa.gov/thredds/ncss/grid/ruc252/"+
	...
	private static MorphVector getRUC_130_MV(String yyyymmddhh, Rectangle2D.Double extent, Date begDate, Date endDate, Date date,
		...
		String ncssURL = "https://nomads.ncdc.noaa.gov/thredds/ncss/grid/ruc130anl/"+
		yyyymmddhh.substring(0, 6)+"/"+yyyymmddhh.substring(0, 8)+"/" +
	...
	private static MorphVector getRUC_252_MV(String yyyymmddhh, Rectangle2D.Double extent, Date begDate, Date endDate, Date date,
		...
		String ncssURL = "https://nomads.ncdc.noaa.gov/thredds/ncss/grid/ruc252anl/"+
		yyyymmddhh.substring(0, 6)+"/"+yyyymmddhh.substring(0, 8)+"/" +
	...
	public static ArrayList<MorphVector> queryGFS_AWS(Date begDate, Date endDate, Rectangle2D.Double extent, StringBuilder operationLog) 
			throws MalformedURLException, IOException, DecodeException, WCTException, WCTNoGridsFoundException {

		SimpleDateFormat sdfIso = new SimpleDateFormat("yyyy-MM-dd'T'HH'%3A'mm'%3A'ss'Z'");
		sdfIso.setTimeZone(TimeZone.getTimeZone("GMT"));

		URL uGfsUrl = new URL("http://noaa-gfs-pds.s3.amazonaws.com/USTRM/6000-0 m HGHT/20180901/0000/003");
		URL vGfsUrl = new URL("http://noaa-gfs-pds.s3.amazonaws.com/VSTRM/6000-0 m HGHT/20180901/0000/003");
		
		return processGeneralGrids(uGfsUrl, vGfsUrl, extent, operationLog);
	}
	
	
	
	public static ArrayList<MorphVector> processGeneralGrids(URL uGrid, URL vGrid, Rectangle2D.Double extent, StringBuilder operationLog) 
			throws MalformedURLException, IOException, DecodeException, WCTException, WCTNoGridsFoundException {
		
		
//		http://noaa-hrrr-pds.s3.amazonaws.com/USTM/0-6000%20m%20HGHT/20180901/0000/000
//		http://noaa-gfs-pds.s3.amazonaws.com/USTRM/6000-0%20m%20HGHT/20180901/0000/003
		
		
		uGrid = WCTTransfer.getURL(uGrid, true);
		GridDatasetRemappedRaster uRaster = new GridDatasetRemappedRaster();
		uRaster.setWidth(100);
		uRaster.setHeight(100);
		uRaster.process(uGrid.toString(), extent);

		
		vGrid = WCTTransfer.getURL(vGrid, true);
		GridDatasetRemappedRaster vRaster = new GridDatasetRemappedRaster();
		vRaster.setWidth(100);
		vRaster.setHeight(100);
		vRaster.process(vGrid.toString(), extent);
		
		ArrayList<MorphVector> mvList = new ArrayList<>(100*100);
		int idx=0;
		
		for (int i=0; i<uRaster.getWidth(); i++) {
			for (int j=0; j<uRaster.getHeight(); j++) {
				
				double u = uRaster.getWritableRaster().getSampleDouble(i, j, 0);
				double v = vRaster.getWritableRaster().getSampleDouble(i, j, 0);
				
				double lon = uRaster.getBounds().getMinY() + i*uRaster.getCellWidth();
				double lat = uRaster.getBounds().getMinY() + j*uRaster.getCellHeight();
