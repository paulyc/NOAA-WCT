    ...
    public void run() {
        ...
        RenderCompleteListener renderCompleteListener = new RenderCompleteListener() {      
            ...
            public void renderProgress(int progress) {
            	...
            	int frameCount = urlsToLoad.length;
            	try {
					frameCount = AnimationUtils.frameCount(viewer, urlsToLoad);
				} catch (NumberFormatException | XPathExpressionException | ParseException | SAXException | IOException
						| ParserConfigurationException e) {
					e.printStackTrace();
				}
            	
//            	System.out.println("frame count for animation: "+frameCount);
            	
            	if (progress > 0 && ++cnt%100 == 0) {
            		double percent = 100.0 * WCTUtils.progressCalculator(
            				new int[] { currentUrlIndexProcessing, progress } , new int[] { frameCount, 100 });
...
//            		System.out.println(percent + ", "+ overallPercentComplete + ", "+progress);
            		...
            		progressBar.setString("Animation Processing Progress: "+WCTUtils.DECFMT_0D00.format(percent)+
            				" % ("+currentUrlIndexProcessing+" / "+ frameCount + ")");
...
//        jButton.setEnabled(false);            
        ...
        JPanel framePreviewLabelPanel = new JPanel();
        framePreviewLabelPanel.add(framePreviewLabel);
        ...
        progressFrame.getContentPane().add(new JScrollPane(framePreviewLabelPanel), "hfill br");
