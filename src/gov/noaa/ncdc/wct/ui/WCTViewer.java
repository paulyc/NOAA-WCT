...
import gov.noaa.ncdc.wct.export.raster.WCTRasterCopy;
...
implements WCTUiInterface, DataDecodeListener, GeneralProgressListener, LoadDataListener, 
ActionListener, WindowListener {
	...
	public static enum CurrentDataType { NONE, RADAR, SATELLITE, MODEL, GRIDDED, LOCATION_STATION };
	...
	private WCTMapPane imgMapPane = new WCTMapPane();
	private boolean isImageDisplayed = false;
	private boolean firstTimeImageDisplayed = true;
//    private ImageZoomPanel izp = new ImageZoomPanel();
	final RenderedGridCoverage imageOrigRGC = new RenderedGridCoverage(
			new GridCoverage("IMAGE_ORIGINAL",
					RasterFactory.createBandedRaster(DataBuffer.TYPE_FLOAT, HEIGHT, WIDTH, 1, null),
					new Envelope(new java.awt.geom.Rectangle2D.Double(179.0, 30.0, .00001, .00001))
			)
	);
	final RenderedGridCoverage imageResampledRGC = new RenderedGridCoverage(
			new GridCoverage("IMAGE_RESAMPLED",
					RasterFactory.createBandedRaster(DataBuffer.TYPE_FLOAT, HEIGHT, WIDTH, 1, null),
					new Envelope(new java.awt.geom.Rectangle2D.Double(179.0, 30.0, .00001, .00001))
			)
	);
...
//	private NexradMath nexradMath;
	...
	private LocationTimeseriesProperties stationPointProps = null;
...
//	final JComboBox<String> jcomboViewSelector = new JComboBox<String>(new String[] { " Standard", " Google Earth Split",});
	final JComboBox<String> jcomboThemeSelector = new JComboBox<String>(new String[] { 
			" Dark", " Light", " Gray", " Streets", " Earth", " World Relief" });
	...
	private JMenuItem jmiLocationSearch, jmiIsosurface, jmiScreenCapture, jmiLoadAnim, jmiCaptureAnim, jmiMath, jmiAttributeFilter, 
	...
	private RenderedLogo locationStationLegend = null;

	private boolean isNOAALogoPainted = true;

	private boolean isGridSatelliteLegendVisible = true;
	private boolean isLocationStationLegendVisible = true;
	...
	private CategoryLegendImageProducer locationStationLegendProducer = new CategoryLegendImageProducer();
...
//		setGlassPane(new WCTHelpGlassPane(this));
	...
	private void createGUI() {
		...
		this.addComponentListener(new WCTResizeListener(this));
...
//		jmiTimeMorphing.setToolTipText("");
		...
		jmiLoadAnim = new JMenuItem("Load Animation", KeyEvent.VK_A);
		jmiLoadAnim.setToolTipText("Load Animation from AVI or Animated GIF");
		...
		jmiPointSubset = new JMenuItem("Point Subset Tool", KeyEvent.VK_P);
		...
		jmiCDOServicesTool = new JMenuItem("NCEI Station Observation Forms");
...
//		jmiGhcn.setToolTipText("Export GHCN (Global Historical Climate Network) data tool.");
		...
		jmiLoadAnim.addActionListener(this);
		// Can't animate what is not there yet
...
//				 TODO change to toggle
		...
		JMenuItem item3 = new JMenuItem("NCEI Station Observation Forms");
...
//		leftPanel.add(appButton);
		...
		jcomboThemeSelector.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					setCurrentTheme(jcomboThemeSelector.getSelectedItem().toString().trim());					
				} catch (Exception ex) {
					ex.printStackTrace();
					//        JOptionPane.showMessageDialog(finalThis, ex.getMessage(), "Theme Change Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
...
//		viewSelectorPanel.add(jcomboViewSelector);
		viewSelectorPanel.add(new JLabel("Theme: "));
		viewSelectorPanel.add(jcomboThemeSelector);
...
//		}
		...
		try {
			...
			servicesManager.addConfig("Data Collections By Organization", url);
		...
		try {
			ServicesMenuManager servicesManager = ServicesMenuManager.getInstance(this, jmDataServicesSubMenu, servicesPopupMenu);		
	        URL url = ResourceUtils.getInstance().getJarResource(
	                new URL(WCTConstants.CONFIG_JAR_URL), ResourceUtils.CONFIG_CACHE_DIR, "/config/dataTypeMenu.xml", null);
			servicesManager.addConfig("Data Types", url);
...
//					System.out.println(lastReleasedMillis);
					if (System.currentTimeMillis() - lastReleasedMillis < 100L) {
						return false;
					}
					lastReleasedMillis = System.currentTimeMillis();					
				}
				
				
				if (finalThis.isFocused() || dataSelect.isFocused() || 
						(gridProps != null && gridProps.isFocused()) || 
						(radialProps != null && radialProps.isFocused()) ) {
				
					if (e.getID() == KeyEvent.KEY_RELEASED && e.isControlDown() &&
							(e.getKeyCode() == KeyEvent.VK_RIGHT || e.getKeyCode() == KeyEvent.VK_DOWN)) {    
						dataSelect.selectNextURL();
						System.out.println("KEY TYPED: "+e.paramString());
						System.out.println(dataSelect.getDataSourcePanel().getDataLocation());
						dataSelect.loadData();
					}
					else if (e.getID() == KeyEvent.KEY_RELEASED && e.isControlDown() &&
							(e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_UP)) {  
						dataSelect.selectPreviousURL();
						System.out.println("KEY TYPED: "+e.paramString());
						System.out.println(dataSelect.getDataSourcePanel().getDataLocation());
						dataSelect.loadData();
					}
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (source == jmiNew) {
			clearAllData();
			setLegendSidePanelVisibility(false);
...
//			markerEditor.setLocation(this.getX()+25, this.getY()+25);
		...
		else if (source == jmiLoadAnim) {

			AnimationFrame af = new AnimationFrame();

			af.setVisible(true);
			af.setLocation(this.getX()+25, this.getY()+25);
//			af.pack();
			af.setExtendedState(Frame.NORMAL);
...
//		}
	...
	public void decodeProgress(DataDecodeEvent decodeEvent) {
		...
		if (decodeEvent.getSource() == goesAreaRaster ||
			...
			// option to show resampling 'live' as rows are completed
//			if (decodeEvent.getProgress() % 10 == 0 &&
//					decodeEvent.getSource() == gridDatasetRaster && ! gridDatasetRaster.isEntireGridCached()) {
//				setGridSatelliteColorTable(mapSelect.getGridColorTableName());
//			}
	...
	public void loadBaseMaps() throws MalformedURLException {
		...
		try {
			...
			try {
				...
				mlPointData = new DefaultMapLayer(pointLayerFeatures, WCTStyleUtils.getDefaultStyle(), "STATION/POINT DATA");
			...
			try {    		
				...
				mlPointDataSelection = new DefaultMapLayer(pointLayerSelectedFeatures, WCTStyleUtils.getDefaultSelectedPointStyle(), "STATION/POINT DATA SELECTED STATION");
			...
			try {
				...
				mlMarkers = new DefaultMapLayer(markerFeatures, MarkerEditor.buildStyle(
						MarkerEditor.getDefaultFont(), MarkerEditor.getDefaultColor()), "MARKER EDITOR");
...
//			JOptionPane.showMessageDialog(this, "already loading...");
		...
		try {
			FileScanner tmpScanner = new FileScanner();
			tmpScanner.scanURL(dataUrl);
//			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.SHAPEFILE) {
			if (dataUrl.toString().startsWith("file") && //dataUrl.toString().endsWith(".shp")) {
					tmpScanner.getLastScanResult().getDataType() == SupportedDataType.SHAPEFILE) {
				mapSelect.loadCustomShapefile(FileUtils.toFile(dataUrl));
				return true;
			}
			
		} catch (DecodeException | SQLException	| ParseException | IOException | NumberFormatException | XPathExpressionException | SAXException | ParserConfigurationException e1) {
			e1.printStackTrace();
		}
		...
		try {
			...
			if (dataUrl == null) {
				System.out.println("dataUrl is null, returning from loadData()");
				WCTUtils.getSharedCancelTask().setCancel(false);
				setIsLoading(false);
				return false;
			}
			...
			if (dataSelect.getSelectedDataType() != SupportedDataType.UNKNOWN) {
				...
				System.out.println("Overriding scanned data type with: "+scannedFile.getLastScanResult().getDataType());
			}
			
			
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.TEXTFILE) {
				
				InputStream in = dataUrl.openStream();
				try {
					WCTTextDialog textDialog = new WCTTextDialog(this, IOUtils.toString(in, "UTF-8"), "Text File");
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.IMAGEFILE) {
				System.out.println("LOADING IMAGE FILE "+dataUrl);
				
				// created a high quality resampled larger image and use native image for zooming in and out and then turn on higher resolution version
				
				statusBar.setProgressText("Resampling Image");
                final BufferedImage bimage = ImageIO.read(dataUrl);                
                int maxSizeLimit = 6000;
				double zoomFactor = 4;
				ResampleOp  resampleOp = new ResampleOp(Math.min((int) (bimage.getWidth()*zoomFactor), maxSizeLimit),
														Math.min((int) (bimage.getHeight()*zoomFactor), maxSizeLimit));
	            resampleOp.setUnsharpenMask(AdvancedResizeOp.UnsharpenMask.Normal);
	            resampleOp.addProgressListener(new ProgressListener() {
					@Override
					public void notifyProgress(float fraction) {
						statusBar.setProgress((int)Math.round(fraction*100));
					}
				});
				BufferedImage resizedImage = resampleOp.filter(bimage, null);
                
                
                GridCoverage gcOrig = new GridCoverage("ImageOriginalGC", bimage, GeographicCoordinateSystem.WGS84, new Envelope(new Rectangle(bimage.getWidth(), bimage.getHeight()) ));
				imageOrigRGC.setGridCoverage(gcOrig);
				imageOrigRGC.setZOrder(1);
				imageOrigRGC.setVisible(true);
				
                GridCoverage gc = new GridCoverage("ImageResampledGC", resizedImage, GeographicCoordinateSystem.WGS84, 
                		new Envelope(new Rectangle.Double(2/zoomFactor, -2/zoomFactor, bimage.getWidth(), bimage.getHeight()) ));
				imageResampledRGC.setGridCoverage(gc);
				imageResampledRGC.setZOrder(2);
				imageResampledRGC.setVisible(true);

				if (resetExtent) {
					imgMapPane.setVisibleArea(gcOrig.getEnvelope().toRectangle2D());
				}
				
				imgMapPane.getRenderer().removeAllLayers();
				imgMapPane.getRenderer().addLayer(imageOrigRGC);
				imgMapPane.getRenderer().addLayer(imageResampledRGC);

				
		        CategoryLegendImageProducer legendProducer = new CategoryLegendImageProducer();
		        legendProducer.setBackgroundColor(new Color(0, 0, 0));
		        legendProducer.setForegroundColor(Color.WHITE);
		        legendProducer.setDrawBorder(false);
		        legendProducer.setLegendTitle(new String[] { scannedFile.getLastScanResult().getFileName() });
		        
		        BufferedImage image = new BufferedImage(1, 1, BufferedImage.TYPE_4BYTE_ABGR);
		        Graphics2D g = (Graphics2D)image.getGraphics();
		        FontRenderContext fc = g.getFontRenderContext();
		        int textHeight = (int) g.getFont().createGlyphVector(fc, scannedFile.getLastScanResult().getFileName()).getVisualBounds().getHeight();
		        int textWidth = (int) g.getFont().createGlyphVector(fc, scannedFile.getLastScanResult().getFileName()).getVisualBounds().getWidth();
		        image = legendProducer.createLargeLegendImage(new Dimension(5000, (int)(2.8*textHeight)));
				RenderedLogo legend = new RenderedLogo(image);
				legend.setPosition(LegendPosition.NORTH_WEST);
                legend.setInsets(new Insets(20, 0, 0, 0));
				legend.setZOrder(500.1f);  
				legend.setAlpha(.75f);
				imgMapPane.getRenderer().addLayer(legend);
		        
		        

				if (firstTimeImageDisplayed) {
				imgMapPane.addZoomChangeListener(new ZoomChangeListener() {	
					TimerTask task = null;
				    Timer timer = new Timer();
				    int waitTime = 1000;   
					
					@Override
					public void zoomChanged(final ZoomChangeEvent evt) {
						
						imageResampledRGC.setVisible(false);
				        ...
				        task = new TimerTask() {
				            public void run() {
//								double overallZoom = bimage.getWidth()/imgMapPane.getVisibleArea().getWidth();
//								System.out.println("executing zoom change:   overall zoom: "+overallZoom);
								imageResampledRGC.setVisible(true);
				            }
				        };
...
//                izp.setImage(dataUrl);
                
				if (!isImageDisplayed) {
					mapPanel.remove(wctMapPane);
//					mainPanel.remove(infoPanel);
					setLegendSidePanelVisibility(false);
					if (vadScrollPane != null) mapPanel.remove(vadScrollPane);
					mapPanel.add(imgMapPane, "Center");
//					mapPanel.add(new JScrollPane(izp), "Center");
					imgMapPane.repaint();
//					izp.repaint();
					mapPanel.repaint();
			...
			if ( ((scannedFile.getLastScanResult().getDataType() == SupportedDataType.UNKNOWN ||
				...
				if (isVADdisplayed || isImageDisplayed) {
					if (vadScrollPane != null) mapPanel.remove(vadScrollPane);
					mapPanel.remove(imgMapPane);
				...
				try {
					...
					if (fd != null && fd.getFeatureType().isPointFeatureType()) {
						...
						stationPointProps = LocationTimeseriesProperties.getInstance(this);
						...
						try {
							...
							mapSelect.isolateLocationStation();								
							lastDecodedLegendProducer = locationStationLegendProducer;
							currentDataType = CurrentDataType.LOCATION_STATION;
							propertiesButton.setEnabled(true);
							propertiesButton.setActionCommand("Location/Station Properties");
							isLocationStationLegendVisible = ! mapSelect.getLocationStationLegendType().trim().equalsIgnoreCase("None");
							return true;

						} catch (Exception e) {
							e.printStackTrace();
							JOptionPane.showMessageDialog(this, 
									"--- Attempting to open Location / Timeseries data -  Data Load Error ---\n"+
											e.getMessage(), 
											"Data Load Error", JOptionPane.ERROR_MESSAGE);
							setIsLoading(false);
							fd.close();
				...
				// if not time series data, clear time series feature collections
				getStationPointFeatures().clear();
				getStationPointSelectedFeatures().clear();
			}
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
				...
				try {
					...
					// override filename timestamp if it exists in the data
					try {
					...
					statusBar.setGridInfo(gridDatasetRaster.getLastProcessedGridCoordSystem(), gridDatasetRaster);
					...
					// set default color table to use
//					scannedFile.getLastScanResult().
...
//							+ "Details: "+cdmLog.toString()
			...
			else if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.IMAGEFILE) {
				System.out.println("FOUND IMAGE FILE (PNG/JPG/GIF)");
			}
			else if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.TEXTFILE) {
				System.out.println("FOUND TEXT FILE (TXT)");
			}
			else {
...
//						"DATA LOADING ERROR", JOptionPane.ERROR_MESSAGE);
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				...
				SampleDimensionAndLabels sd = 
					NexradSampleDimensionFactory.getSampleDimensionAndLabels(
							level3Header.getProductCode(), false, level3Header.getVersion());
				radLegendProducer.setSampleDimensionAndLabels(sd);
...
//        			"Data Load Error", JOptionPane.ERROR_MESSAGE);
			...
			if (e.getMessage().startsWith("Operation canceled")) {
				...
				e.printStackTrace();
				LocationTimeseriesProperties.getInstance(this).setVisible(false);
	        	JOptionPane.showMessageDialog(this, 
	        			"--- Data Load Error ---\n"+
...
//	        			"Specific Error: "+e.getMessage()+
	...
	private RenderedLogo createGridSatelliteLegend() {
		RenderedLogo legend = new RenderedLogo();
		legend.setPosition(LegendPosition.SOUTH_EAST);
		legend.setZOrder(500.1f);                    

		return legend;
	}

	private RenderedLogo createLocationStationLegend() {
		RenderedLogo legend = new RenderedLogo();
		legend.setPosition(LegendPosition.SOUTH_EAST);
		legend.setZOrder(500.2f);                    
	...
	public LocationTimeseriesProperties getStationPointProperties() {
...
//				System.out.println("getCurrentExtent: "+getCurrentExtent());
				...
				gridDatasetRaster.setWidth(mapPanel.getWidth());
				gridDatasetRaster.setHeight(mapPanel.getHeight());
				...
				// The idea here is for a default color table to be used, if specified in the filename config file or product lookup table.
				// But, hold onto the previous color table used if the dataset is changed to something that doesn't have a default lookup table.
				// Also, if we change the custom range of something with a default color table, don't reset range to the default until that
				//  default color table is changed to a different table.
				
				
				String ctName = mapSelect.getGridColorTableName();
				String defaultColorTable = scannedFile.getLastScanResult().getDefaultColorTable();
				if (ctName.equalsIgnoreCase("Default") && defaultColorTable != null) {
					
					// flip and range are included in this string - parse them out...
					// Rainbow;flip;20:40
					String[] vals = defaultColorTable.split(";");
					boolean isFlipped = false;
					boolean isCustomRange = false;
					int min=0, max=0;
					
					for (int n=1; n<vals.length; n++) {
						if (vals[n].trim().equalsIgnoreCase("flip")) {
							isFlipped = true;
						}
						if (vals[n].contains(":")) {
							try {
								String[] minMaxStrings = vals[n].split(":");
								min = Integer.parseInt(minMaxStrings[0].trim());
								max = Integer.parseInt(minMaxStrings[1].trim());
								isCustomRange = true;
					...
					gridDatasetRaster.setAutoMinMaxValues(! isCustomRange);
					mapSelect.setGridAutoMinMax(! isCustomRange);
					// if the default color table hasn't changed, don't reset color table range
					if (isCustomRange && ! ctName.equals(vals[0])) {
						gridDatasetRaster.setDisplayMinValue(Math.min(min, max));
						gridDatasetRaster.setDisplayMaxValue(Math.max(min, max));
						mapSelect.setGridColorTableMinMaxValue(min, max);
					}
//					mapSelect.setGridColorTableName(vals[0]);
					mapSelect.setGridColorTableFlipped(isFlipped);

					if (vals[0].startsWith("Satellite: ") || vals[0].startsWith("Radar: ") || vals[0].startsWith("Drought: ")) {
						ColorsAndValues cav = GridDatasetColorFactory.getColorsAndValues(
								vals[0].substring(vals[0].indexOf(" ")+1, vals[0].length()));
						
						gridDatasetRaster.setDisplayColors(cav.getColors());
					}
					else {
						gridDatasetRaster.setDisplayColors(GridDatasetColorFactory.getColors(vals[0], isFlipped));  
					}			
					
				}
				else {
					
					// if the UI has selected "Default", use the Rainbow color table if a default isn't specified in the config.
					if (ctName.equalsIgnoreCase("Default")) {
						ctName = "Rainbow";
					}
					
					if (ctName.startsWith("Satellite: ") || ctName.startsWith("Radar: ") || ctName.startsWith("Drought: ")) {
						ColorsAndValues cav = GridDatasetColorFactory.getColorsAndValues(
						...
						gridDatasetRaster.setDisplayColors(GridDatasetColorFactory.getColors(ctName, mapSelect.isGridColorTableFlipped()));  
					}
				}
...
//		gridSatLegendProducer.setDataDescription(new String[] { scannedFile.getLastScanResult().getFileName() });
		gridSatLegendProducer.setDataDescription(new String[] { scannedFile.getLastScanResult().getLongName() });
		SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm");
		dateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
		String dateTimeString = null;
		try {
			if (gridDatasetRaster.getDateInMilliseconds() != Long.MIN_VALUE) {
				dateTimeString = dateFormat.format(gridDatasetRaster.getDateInMilliseconds())+" Z";
				gridSatLegendProducer.setDateTimeInfo(new String[] { dateTimeString });
			}
		} catch (Exception e) {
...
//		System.out.println(Arrays.toString(catValues));
		...
		ColorsAndValues cav1 = new ColorsAndValues(catColors, catValues, gridDatasetRaster.getUnits());
...
//					scannedFile.getLastScanResult().getFileName(),
					scannedFile.getLastScanResult().getLongName(),
...
//			gridSatLegendProducer.setLegendTitle(legendTitleArray);
		...
		gridSatLegendProducer.setMapBackgroundColor(this.getMapBackgroundColor());
	...
	public void refreshGridOrSatellite() {
		refreshGridOrSatellite(false);
	}

	public void refreshGridOrSatellite(boolean resetExtent) {
	...
	public void refreshSatelliteLegend(Date date, String dateNote) throws Exception {
		...
		dateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
//		gridSatLegendProducer.setDateTimeInfo(dateFormat.format(goesAreaRaster.getDateInMilliseconds())+" Z");
		gridSatLegendProducer.setDateTimeInfo(new String[] { dateFormat.format(date)+" Z" });
...
//						scannedFile.getLastScanResult().getTimestamp() });
						dateFormat.format(date)+" Z "+dateNote });
				gridSatLegendProducer.setDrawColorMap(true);
				gridSatLegendProducer.setDrawLabels(false);
...
//		frame.setVisible(true);
	...
	public boolean getGridSatelliteLegendVisibility() {
		return this.isGridSatelliteLegendVisible;
	}

	public void setLocationStationLegendVisibility(boolean isVisible) {
		isLocationStationLegendVisible = isVisible;
		if (locationStationLegend != null) {
			locationStationLegend.setVisible(isVisible);
			locationStationLegend.repaint();
			wctMapPane.repaint();
		}
	}
	public boolean getLocationStationLegendVisibility() {
		return this.isLocationStationLegendVisible;
	}
	...
	public void refreshStationDatasetLegend() throws Exception {

		locationStationLegendProducer.setDataType("STATION DATA");
		locationStationLegendProducer.setDataDescription(new String[] { scannedFile.getLastScanResult().getFileName() });
		SimpleDateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm");
		dateFormat.setTimeZone(TimeZone.getTimeZone("GMT"));
		String dateTimeString = null;
		try {
//			if (stationPointProps.getCurrentDataSliceDateString() != Long.MIN_VALUE) {
//				dateTimeString = dateFormat.format(stationPointProps.getCurrentDataSliceDateString())+" Z";
			if (stationPointProps.getCurrentDataSliceDateString() != null) {
				dateTimeString = stationPointProps.getCurrentDataSliceDateString();
				locationStationLegendProducer.setDateTimeInfo(new String[] { dateTimeString });
			}
		} catch (Exception e) {
			// no date or time info present for this file
		}
		
		System.out.println("current var (viewer): "+stationPointProps.getCurrentVariable());
		String unitsString = stationPointProps.getCurrentVariable().getUnitsString();
		if (unitsString == null) {
			unitsString = "unitless";
		}
		
		String varAndUnitsString = stationPointProps.getCurrentVariable().getShortName()+" ("+unitsString+")";


//		System.out.println(gridSatLegendProducer.getDateTimeInfo());

		Color[] catColors = LegendCategoryFactory.getCategoryColors(stationPointProps);
		Double[] catValues = LegendCategoryFactory.getCategoryValues(stationPointProps);

//		System.out.println(Arrays.toString(catValues));

		// don't send in units, yet...
		ColorsAndValues cav1 = new ColorsAndValues(catColors, catValues, null);
		if (cav1.getValues().length > 1 && cav1.getValues()[1] > cav1.getValues()[0]) {
			cav1.flip();
		}
		ColorsAndValues cav = ColorsAndValues.calculateEqualColorsAndValues( cav1, 51);
		...
		if (pointLayerFeatures.size() == 0) {
			locationStationLegendProducer.setLabelOverride(" No Data ");
		}
		else {
			locationStationLegendProducer.setLabelOverride("");
		}

		locationStationLegendProducer.setCategoryColors(cav.getColors());        
		locationStationLegendProducer.setCategoryLabels(catLabels);
		locationStationLegendProducer.setInterpolateBetweenCategories(true);
...
//					+ ((heightString == null) ? "" : heightString)
			...
			LegendEditor legendEditor = LegendEditor.getInstance(this, this);
			if (! legendEditor.isEngageCustomLegendOverride()) {
				locationStationLegendProducer.setLegendTitle(legendTitleArray);
			}
			//            System.out.println("legend ------ "+Arrays.toString(legendTitleArray));

//			locationStationLegendProducer.setLegendTitle(legendTitleArray);
			locationStationLegendProducer.setDrawColorMap(true);
			locationStationLegendProducer.setDrawLabels(true);
			locationStationLegendProducer.setLabelEveryOtherN(10);

		} catch (Exception e) {
			e.printStackTrace();
			locationStationLegendProducer.setLegendTitle(new String[] { scannedFile.getLastScanResult().getFileName() });
			locationStationLegendProducer.setDrawColorMap(false);
			locationStationLegendProducer.setDrawLabels(false);
		}


		locationStationLegendProducer.setBackgroundColor(new Color(220, 220, 220, 210));
		locationStationLegendProducer.setMapBackgroundColor(this.getMapBackgroundColor());
		locationStationLegendProducer.setDrawBorder(true);
		locationStationLegendProducer.setInterpolateBetweenCategories(true);
		lastDecodedLegendProducer = locationStationLegendProducer;


		Image image = locationStationLegendProducer.createMediumLegendImage();
		if (locationStationLegend == null) {
			locationStationLegend = createLocationStationLegend();
			wctMapPane.getRenderer().addLayer(locationStationLegend);
		}
		locationStationLegend.setInsets(new Insets(0, 0, 15, image.getWidth(this)));
		locationStationLegend.setImage(image);
		locationStationLegend.setVisible(true);
...
//		}
	...
	public void setLocationStationColorTable(String colorTableAlias) {

		int alphaPcnt = mapSelect.getLocationStationTransparency();
		if (alphaPcnt < 0) {
			alphaPcnt = LocationTimeseriesProperties.DEFAULT_ALPHA_PERCENT; 
		}
		int alpha = 255-(int)(alphaPcnt/100.0*255);
		setLocationStationColorTable(colorTableAlias, alpha);
	}
	
	public void setLocationStationColorTable(String colorTableAlias, int alpha) {
//		if (currentDataType == CurrentDataType.LOCATION_STATION) {
			
			try {
				Color[] c = GridDatasetColorFactory.getColors(colorTableAlias, false);
				if (mapSelect.isLocationStationColorTableFlipped()) {
					WCTUtils.flipArray(c);
				}
				
				double minVal, maxVal;
				if (mapSelect.isLocationStationAutoMinMaxSelected()) {
					minVal = stationPointProps.getLastProcessedMinValue();
					maxVal = stationPointProps.getLastProcessedMaxValue();
					mapSelect.setLocationStationColorTableMinMaxValue(minVal, maxVal);
				}
				else {
					minVal = mapSelect.getLocationStationColorTableMinValue();
					maxVal = mapSelect.getLocationStationColorTableMaxValue();
				}
				mlPointData.setStyle(WCTStyleUtils.getStyle(stationPointProps.getCurrentFeatureType(),
						stationPointProps.getCurrentStationFeatureAttribute(), 
						c, minVal, maxVal, stationPointProps.isCurrentPolygonLookup(), alpha));
				ColorsAndValues cav = WCTStyleUtils.getColorsAndValues(c, minVal, maxVal);
				stationPointProps.setCurrentSymbolizedColors(cav.getColors());
				stationPointProps.setCurrentSymbolizedValues(cav.getValues());
				
				refreshStationDatasetLegend();
...
//		}
	...
	public void setLocationStationTransparency(int alpha) {
		setLocationStationColorTable(mapSelect.getLocationStationColorTableName(), alpha);
	}
	...
	public void setRadarTransparency(int alphaChannelValue, WCTRasterizer rasterizer) {
		try {
			...
			if (radarRGC != null && scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				...
				mapSelect.updateRadarColorTable();
...
//				else {
//				
//					radarGC = WCTGridCoverageSupport.applyAlpha2(radarGC, radarAlphaChannelValue, gcSupport.getCachedSampleDimensions(), AlphaInterpolationType.NONE);
//				}
//				
//				
//				radarRGC.setGridCoverage(radarGC);
...
//					radarGC = WCTGridCoverageSupport.applyAlpha2(radarGC, gcSupport.getCachedSampleDimensions(), radarAlphaChannelValue);
//					radarRGC.setGridCoverage(radarGC);
...
//					radarRGC.setGridCoverage(radarGC);
	...
	public void snapshotCurrentLayer() {
		...
		if (currentDataType == CurrentDataType.RADAR) {
			...
			if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL) {
				raster = WCTRasterCopy.getInstance(radialDatasetRaster);
			}
			else {
				raster = WCTRasterCopy.getInstance(getWCTRasterizer());
			}
			...
			try {
				if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
					...
					raster = WCTRasterCopy.getInstance(getGoesRaster());
				}
				else {
					units = getGridDatasetRaster().getUnits();
					isResampled = ! getGridDatasetRaster().isNative();
					raster = WCTRasterCopy.getInstance(getGridDatasetRaster());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//		CategoryLegendImageProducer legendProducer = null;
	...
	public MarkerEditor showMarkerEditor() {
		...
		if (markerEditor == null) {
			...
			markerEditor.addStyleChangeListener(new StyleChangeListener() {
				@Override
				public void styleChanged(Style newStyle) {
					mlMarkers.setStyle(newStyle);
				}
			});
		}
		markerEditor.setVisible(true);
	...
	public DefaultMapLayer getStationPointSelectedMapLayer() {
		return mlPointDataSelection;
	}
	...
	public void setCurrentDataTypeOverride(CurrentDataType dataType) {
...
//	}
	...
	public CategoryLegendImageProducer getLocationStationLegendImageProducer() {
		return this.locationStationLegendProducer;
	}
...
//	}
	...
	public void clearData() {
		try {
			...
			setLocationStationLegendVisibility(false);
			setLegendSidePanelVisibility(false);
	...
	public void setLayerFillColor(int index, Color newColor) {
		try {
			...
			if (index == STATES) {
				...
				System.out.println("resetting halo color to: "+newColor);
...
//					progressBar.setString("Updating Style "+n+" of "+NUM_LAYERS+" Layers");
					statusBar.setProgress(100*n/NUM_LAYERS);
					statusBar.setProgressText("Updating Style "+n+" of "+NUM_LAYERS+" Layers");
...
//			setLayerVisibility(index, !isLayerVisible(index));
	...
	public void setLayerVisibility(int index, boolean vis) {
		try {
			...
			((MapLayer) baseMapLayers.elementAt(index)).setVisible(vis);
			// Add layer if not already added to context.
...
//			System.out.println("SETTING LABEL " + index + " TO " + vis);
			...
			if (vis && map.indexOf((MapLayer)baseMapLabelLayers.elementAt(index)) == -1) {
				...
				map.addLayer(100+index, new DefaultMapLayer(org.geotools.feature.FeatureCollections.newCollection(), sb.createStyle()));
			}
	...
	public void setLegendFont(Font font) throws Exception {
		...
		locationStationLegendProducer.setFont(font);
		refreshLegend();
	}
	...
	public void setLegendFGColor(Color fgcolor) throws Exception {
		...
		locationStationLegendProducer.setForegroundColor(fgcolor);
		refreshLegend();
	}
	...
	public void setLegendBGColor(Color bgcolor) throws Exception {
		...
		locationStationLegendProducer.setBackgroundColor(bgcolor);
		refreshLegend();
	}

	/**
	 * Refreshes the Radar (large) or Grid/Satellite (medium sized) legend.  This is needed
	...
	public void refreshLegend() throws Exception {
		...
		else if (currentDataType == CurrentDataType.LOCATION_STATION) {
			Image image = locationStationLegendProducer.createMediumLegendImage();
			locationStationLegend.setInsets(new Insets(0, 0, 15, image.getWidth(this)));
			locationStationLegend.setImage(image);
			locationStationLegend.setVisible(isLocationStationLegendVisible);
		}
		else {
			Image image = gridSatLegendProducer.createMediumLegendImage();
			gridSatelliteLegend.setInsets(new Insets(0, 0, 15, image.getWidth(this)));
	 ...
	 *
	 * @param  useWctCache  The new useWctCache value
	 */
...
//					mapPanel.getLocationOnScreen().x, mapPanel.getLocationOnScreen().y, exportWidth, exportHeight);
	 ...
	 */
	public void setLegendSidePanelVisibility(boolean showLegend) {
...
//		statusBar.setText("Memory Usage: "+ ((int)(((double)(r.totalMemory() - r.freeMemory()) / maxMemory)*100.0)) + "%");
		...
		statusBar.setText("Memory Usage: "+ ((int)(((double)(r.totalMemory() - r.freeMemory()) / maxMemory)*100.0)) + "% ["+
				r.totalMemory()/1024/1024+" / "+r.maxMemory()/1024/1024 + " MB]");
		
//		System.out.printf("%s %s %s", r.maxMemory()/1024/1024, r.totalMemory()/1024/1024, r.freeMemory()/1024/1024);
//		System.out.println();
	}
...
//			}
	...
	public void setCurrentTheme(String themeName) {
		...
		SwingWorker worker = new SwingWorker() {
			public Object construct() {
				...
				ThemeManager.setViewerTheme(finalThis, themeName);
				return "Done";
			}
		};
		worker.start();
	...
	private void setCurrentViewTypeValue(CurrentViewType viewType) throws BindException, Exception {
		...
		if (viewType == CurrentViewType.GOOGLE_EARTH || viewType == CurrentViewType.NCDC_NCS ||
			...
			if (viewType == CurrentViewType.GOOGLE_EARTH) {
				...
				setLegendSidePanelVisibility(getLegendVisibility());
	...
	public void refreshAfterResize() {
		...
		System.out.println("COMPONENT RESIZED - TRYING TO RESET RASTERIZER TO: "+wctMapPane.getSize().toString());
		...
		if (rasterizer != null && ( rasterizer.getHeight() != wctMapPane.getHeight() 
			...
			System.out.println("rasterizer dims: "+rasterizer.getWidth()+" x "+rasterizer.getHeight());
			System.out.println("map pane dims: "+wctMapPane.getWidth()+" x "+wctMapPane.getHeight());
			...
			//            }
			//            else {
			rasterizer = new WCTRasterizer((int)(wctMapPane.getHeight()*WCTViewer.RASTER_SIZE_FACTOR), (int)(wctMapPane.getWidth()*WCTViewer.RASTER_SIZE_FACTOR));
			rasterizer.setNoDataValue(Double.NaN);
...
//			System.out.println("   ---- Done ");
			...
			System.out.println("DISABLING NETCDF-JAVA FILE CACHE - leaves open file handles which causes issues.");
			NetcdfDataset.initNetcdfFileCache(1, 1, 1);
