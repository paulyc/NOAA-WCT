...
public class GridDatasetProperties extends JDialog {
    ...
    private ArrayList<LoadDataListener> listeners = new ArrayList<LoadDataListener>();
...
//        zPanelParent.setBorder(BorderFactory.createLineBorder(Color.RED));
        ...
        MouseAdapter doubleClickToLoadListener = new MouseAdapter() {
			@Override
        ...
        KeyAdapter enterToLoadListener = new KeyAdapter() {
        	@Override
        ...
        gridJList.addKeyListener(enterToLoadListener);
	    timeJList.addKeyListener(enterToLoadListener);
	    rtJList.addKeyListener(enterToLoadListener);
	    zJList.addKeyListener(enterToLoadListener);
	    gridJList.addMouseListener(doubleClickToLoadListener);
	    timeJList.addMouseListener(doubleClickToLoadListener);
	    rtJList.addMouseListener(doubleClickToLoadListener);
	    zJList.addMouseListener(doubleClickToLoadListener);
...
//        String[] tzIdArray = TimeZone.getAvailableIDs();
        DefaultComboBoxModel<String> ids = new DefaultComboBoxModel<String>();
        ids.addElement("Default ("+TimeZone.getDefault().getDisplayName(true, TimeZone.SHORT)+")");
        ids.addElement("UTC");
        ids.addElement("Both");
        final JComboBox<String> jcbTimeFormat = new JComboBox<String>(ids);
        ...
        tabRenderer.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
...
//                System.out.println("   z valueChanged: firstIndex: "+e.getFirstIndex());
    ...
    private JXHyperlink getMoreLink() {
        ...
        item7.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
    			new RasterMathDialog(viewer, finalThis);
            }
        });
        morePopupMenu.add(item7);

        JMenuItem item8 = new JMenuItem("Point Subset Tool");
        item8.setToolTipText("Display Point Subset Tool");
        item8.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
    			new PointSubsetDialog(viewer, finalThis);
            }
        });
        ...
        itemNcdump.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                DataSelectionUtils.showDump(viewer, viewer.getDataSelector(), gridDatasetURL);
            }
        });
        ...
        itemCollectionMetadata.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                DataSelectionUtils.browseToCollectionMetadata(viewer, viewer.getDataSelector(), gridDatasetURL);
            }
        });
...
//            return;
        ...
        DefaultListModel<String> rtListModel = new DefaultListModel<String>();
        if (grid.getCoordinateSystem().getRunTimeAxis() != null) {
            List<CalendarDate> dates = grid.getCoordinateSystem().getRunTimeAxis().getCalendarDates();
            for (CalendarDate date : dates) {
                rtListModel.addElement(sdf.format(date.toDate()));
            }
        }
...
//            return;
        ...
        if (grid.getCoordinateSystem().hasTimeAxis1D()) {
        	List<CalendarDate> dates = grid.getCoordinateSystem().getTimeAxis1D().getCalendarDates();
            for (CalendarDate date : dates) {
                String timeString = null;
                if (selectedTimeZoneIndex == 0) {
                    timeString = sdf2.format(date.toDate());
                }
                else if (selectedTimeZoneIndex == 1) {
                    timeString = sdf.format(date.toDate());
                }
                else {
                    timeString = sdf2.format(date.toDate()) + " ("+sdf.format(date.toDate())+")";    
                }
...
//            return;
        ...
        DefaultListModel<String> zListModel = new DefaultListModel<String>();
        if (grid.getCoordinateSystem().getVerticalAxis() != null) {
        	try {
//        		double[] zVals = grid.getCoordinateSystem().getVerticalAxis().getCoordValues();
