			...
			if (isOperationAcrossDimensions) {
				...
				for (int n=0; n<selectedIndices.length; n++) {
					...
					if (viewer.getRadialProps() != null) {
						exporter.setExportVariable(viewer.getRadialProps().getRadialPropsPanel().getVariableName());
						exporter.setExportCut(viewer.getRadialProps().getRadialPropsPanel().getCut());
						exporter.setRadialExportCappiHeightsInMeters(
			            		Double.isNaN(viewer.getRadialProps().getRadialPropsPanel().getCappiAltitude()) 
		            			? null : new double[] { viewer.getRadialProps().getRadialPropsPanel().getCappiAltitude() });
								
						exporter.setRadialExportCappiInterpolationType(viewer.getRadialProps().getRadialPropsPanel().getCurrentCappiType());
					}
...
//        JOptionPane.showMessageDialog(this, ex.getMessage());
	...
	private void showExportDialog() {
		...
		mainPanel.setBorder(WCTUiUtils.myTitledTopBorder("Select Export Format and File", 0, 5, 2, 5, TitledBorder.CENTER, TitledBorder.TOP));
