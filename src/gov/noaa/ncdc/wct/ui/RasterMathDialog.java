...
//			viewer.setCurrentDataTypeOverride(CurrentDataType.GRIDDED);
		}
		else {
...
//			viewer.getLegendPanel().repaint();
			...
			legendProducer.setDateTimeInfo(new String[] { "MATH OPERATION RESULTS" });
	...
	private GridCoverage getGridCoverageFromProcessedRaster(WCTExport exporter, WCTRaster processRaster) throws Exception {
		...
		if (dataType == SupportedDataType.GOES_SATELLITE_AREA_FORMAT) {
			...
			grid.setBounds(processRaster.getBounds());
...
//                formSubmitted = true;
            	...
            	quickExport.showExportRasterDialog(parent, processRaster);
            }
