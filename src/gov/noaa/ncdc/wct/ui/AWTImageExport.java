...
public class AWTImageExport {
    ...
    public File exportAWTChooser(Component parent, String startPath) {
        ...
        for (int i = 0; i < frameCount; i++) {
            ...
            OpenFileFilter pngFilter = new OpenFileFilter("png", true, "PNG Images (72 DPI)");
            OpenFileFilter png150Filter = new OpenFileFilter("png", true, "PNG Images (150 DPI)");
            OpenFileFilter png300Filter = new OpenFileFilter("png", true, "PNG Images (300 DPI)");
            OpenFileFilter png1200Filter = new OpenFileFilter("png", true, "PNG Images (1200 DPI)");
            ...
            fc.addChoosableFileFilter(png150Filter);
            fc.addChoosableFileFilter(png300Filter);
            fc.addChoosableFileFilter(png1200Filter);
...
//                File extfile = null;
                ...
                if (fc.getFileFilter() == jpgFilter) {
                	file = new File(checkExtension(fstr, "jpg"));
                }
                else if (fc.getFileFilter() == gifFilter) {
                	file = new File(checkExtension(fstr, "gif"));
                }
                else if (fc.getFileFilter() == pictFilter) {
                	file = new File(checkExtension(fstr, "pict"));
                }
                else if (fc.getFileFilter() == tiffFilter) {
                	file = new File(checkExtension(fstr, "tiff"));
                }
                else if (fc.getFileFilter() == bmpFilter) {
                	file = new File(checkExtension(fstr, "bmp"));
                }
                else if (fc.getFileFilter() == targaFilter) {
                	file = new File(checkExtension(fstr, "tga"));
                }
                else {
                	file = new File(checkExtension(fstr, "png"));
                }
                ...
                if (choice == JOptionPane.YES_OPTION) {
                    ...
                    else if (fc.getFileFilter() == png150Filter) {
                        exportImage(file, i, Type.PNG, 150);
                    }             
                    else if (fc.getFileFilter() == png300Filter) {
                        exportImage(file, i, Type.PNG, 300);
                    }             
                    else if (fc.getFileFilter() == png1200Filter) {
                        exportImage(file, i, Type.PNG, 1200);
                    }
                    else {
    ...
    public boolean exportImage(File file, int index, Type outType) {
        ...
        return (saveImage(awtImages[index], file, outType, 72));
    }
    
    
    public boolean exportImage(File file, int index, Type outType, int dpi) {

        return (saveImage(awtImages[index], file, outType, dpi));
    }
...
//     * @return          Description of the Return Value
//     */
//    public static boolean saveImage(BufferedImage image, File outFile, Type outType, int dpi) {
//    	try {
//    		
//            return true;
//        } catch (Exception e) {
...
//    }
    ...
    public static boolean saveImage(java.awt.Image image, File outFile, Type outType) {
    	return saveImage(image, outFile, outType, 72);
    }
     ...
     * @return          Description of the Return Value
     */
    public static boolean saveImage(java.awt.Image image, File outFile, Type outType, int dpi) {
    	return saveImage(AWTImageExport.toBufferedImage(image), outFile, outType, dpi);
    }
    ...
    public static boolean saveImage(BufferedImage image, File outFile, Type outType, int dpi) {
        try {
            ...
            if (outType == Type.PNG) {
            	if (dpi == 72) {
                    ImageIO.write(image, "png", new File(checkExtension(outFile.toString(),".png")));
            	}
            	else {
            		ImageExportUtils.saveImage(image, "png", new File(checkExtension(outFile.toString(),".png")), dpi);
            	}
            }
            	...
            	if (dpi == 72) {
            		ImageIO.write(image, "jpg", new File(outFile.toString() + ".jpg"));
            	}
            	else {
            		ImageExportUtils.saveImage(image, "jpeg", new File(checkExtension(outFile.toString(),".jpeg")), dpi);
            	}
    ...
    public void saveAsMovie(Type outFormat, File outFile, int delayBetweenFramesInMillis, JProgressBar progressBar) {
        ...
        int delayAtEndBeforeRepeat = 800;
        int numberOfFramesToAddAtEnd = delayAtEndBeforeRepeat/delayBetweenFramesInMillis;
        ...
        if (outFormat == Type.AVI) {
            ...
            try {
                ...
                for (int n=1; n<frameCount; n++) {
                    ...
                    if (awtImages == null) {
                        bimage = toBufferedImage(imageIterator.next());
                    }
                    else {
                        bimage = toBufferedImage(awtImages[n]);
                    }
                    aviWriter.addFrame(bimage);
                }
                
                // 4. add extra frames to simulate a delay before repeating
                for (int n=0; n<numberOfFramesToAddAtEnd; n++) {
                    progressBar.setString("Repeating Final Frame " + n + " of " + numberOfFramesToAddAtEnd);
                    progressBar.setValue(n);
                    aviWriter.addFrame(bimage);
                }
