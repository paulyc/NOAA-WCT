...
//        });
        ...
        JButton loadL2Radar = new JButton("Load Radar Data (Level-II)");
        ...
        JButton orderL3Radar = new JButton("Order Radar Products (Level-III)");
...
//        animationOrder.setEnabled(false);
    ...
    private void loadL2Radar() throws Exception {
        ...
        BDPAccessPanel bdpAccessPanel = viewer.getDataSelector().getDataSourcePanel().getBDPAccessPanel();
        bdpAccessPanel.setBDPDataset("NEXRAD Level-II");
        bdpAccessPanel.getNexradBDPAccessPanel().setDate(dateString.substring(0, 4) + dateString.substring(5, 7) + dateString.substring(8, 10));
        bdpAccessPanel.getNexradBDPAccessPanel().setSite(closestId);
...
//    	viewer.setCurrentExtent(WCTUtils.toRectangle(hailReportsMapLayer.getFeatureSource().getBounds()));
        ...
        logo.setPosition(LegendPosition.NORTH_EAST);
        logo.setInsets(new Insets(img.getHeight(null)+18, 0, 18, img.getWidth(null)));
