...
public class SearchDialog extends JDialog {
	...
	private WCTGeocoder googleGeocoder = new WCTGeocoder();
...
//		dataInfoSearchPanel.add("br", new JLabel(" "));
	...
	private void locationSearch(String location) {
		try {
			...
			List<GeocodeResult> results = googleGeocoder.locationSearch(location, viewer.getCurrentExtent());
			if (results.size() == 0) {
				JOptionPane.showMessageDialog(this, "No results found from the Geocoding Service.  \n" +
						"Please validate input search location or zoom out.  Search results are limited to the current view extent.", 
						"Geocoding Service: No results found", JOptionPane.ERROR_MESSAGE);
				return;
			}
			GeocodeResult ggr = results.get(0);
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Error using Geocoding Service.  " +
