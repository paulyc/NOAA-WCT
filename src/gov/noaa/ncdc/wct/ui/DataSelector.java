...
import gov.noaa.ncdc.common.DisabledPanel;
...
public class DataSelector extends JXDialog implements ScanResultsManager, ActionListener, ListSelectionListener {
    ...
    private DefaultComboBoxModel<String> autoReloadModel = 
            new DefaultComboBoxModel<String>(new String[] { " Off ", " 30 sec", " 1 min", " 3 min", " 5 min", " 10 min", " 15 min", " 20 min", " 30 min", " 45 min", " 60 min", " 120 min" });
    private JComboBox<String> jcomboAutoReload = new JComboBox<String>(autoReloadModel);

    private Timer reloadTimer = null; 
    private JLabel reloadTimerLabel = new JLabel("");
    private JLabel reloadLabel = new JLabel("  Auto-Reload: ");
        ...
        public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected,
            ...
            if (! dataSourcePanel.getDataType().equals(WCTDataSourceDB.SINGLE_FILE) &&
            	...
            	try {
            		...
            		else if (viewer.isUseWctCache() && scanResultsCacheStatus != null && index < scanResultsCacheStatus.length && 
            				scanResultsCacheStatus[resultsList.convertIndexToModel(index)]) {
...
//        super(viewer, "Data Selector", false);
    ...
    public void pack() {
        ...
        setSize(new Dimension( ( super.getPreferredSize().width > 950 ) ? 950 : super.getPreferredSize().width, super.getPreferredSize().height));
    }
...
//        });
        ...
        itemDownload.setToolTipText("Opens the data export tool");
        ...
        JMenuItem itemNcdump = new JMenuItem("File Metadata (ncdump)");
        itemNcdump.setToolTipText("Display file metadata (ncdump) for file/url");
        ...
        JMenuItem itemCollectionMetadata = new JMenuItem("Dataset Documentation (DOI)");
        itemCollectionMetadata.setToolTipText("Browse to the collection metadata and documentation for this dataset");
        itemCollectionMetadata.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                DataSelectionUtils.browseToCollectionMetadata(viewer, finalThis, getSelectedURLs()[0]);
            }
        });
        ...
        JMenuItem itemPointSubsetTool = new JMenuItem("Point Subset Tool");
        ...
        morePopupMenu.add(itemDownload);
        morePopupMenu.add(itemShowInFolder);
        morePopupMenu.add(itemCollectionMetadata);
...
//        morePopupMenu.add(itemOpenPanoply2);
        ...
        JPanel cacheIconPanel = new JPanel();
        cacheIconPanel.add(cacheLink);
        cacheIconPanel.add(new JLabel(""));
        cacheIconPanel.add(new JLabel(redIcon));
        cacheIconPanel.add(new JLabel("/"));
        cacheIconPanel.add(new JLabel(greenIcon));
        ...
        if (! dataSourcePanel.getDataType().equals(WCTDataSourceDB.LOCAL_DISK)) {
            instructionPanel.add(cacheIconPanel, BorderLayout.WEST);
        }
        else {
            instructionPanel.remove(cacheIconPanel);
        }
...
//        choicePanel.add(emptyPanel, "right");
        ...
        choicePanel.add(reloadLabel);
        choicePanel.add(jcomboAutoReload);
        choicePanel.add(reloadTimerLabel);
...
//        choicePanel.add(jbExport);
        ...
        jcomboAutoReload.addPropertyChangeListener("enabled", new PropertyChangeListener() {
            @Override
            public void propertyChange(PropertyChangeEvent evt) {
                System.out.println("State changed for " + evt.getPropertyName() + " to " + evt.getNewValue());
                System.out.println("jcomboAutoReload text color: "+jcomboAutoReload.getForeground());
            }
        });
        
        
        
        jcomboAutoReload.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent evt) {
				...
				if (jcomboAutoReload.getSelectedItem().toString().contains("Off")) {
					reloadTimerLabel.setText("");
					viewer.getStatusBar().setProgressText("");
					lockInterface(false);
					setIsLoading(false);
					jcomboAutoReload.setEnabled(true);
					return;
				}
				else if (jcomboAutoReload.getSelectedItem().toString().contains("sec")) {
					lockInterface(true);
					setIsLoading(true);
					jcomboAutoReload.setEnabled(true);
					duration = Long.parseLong(jcomboAutoReload.getSelectedItem().toString().replace("sec", "").trim());
				}
				else if (jcomboAutoReload.getSelectedItem().toString().contains("min")) {
					lockInterface(true);
					setIsLoading(true);
					jcomboAutoReload.setEnabled(true);
					duration = Long.parseLong(jcomboAutoReload.getSelectedItem().toString().replace("min", "").trim());
					duration *= 60;
				}
				
				createAutoReloadTimer(duration*1000L);
			}
		});
...
//
        JXHyperlink shortcutLink = new JXHyperlink();
        shortcutLink.setText("Keyboard Shortcut:");
        shortcutLink.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String str = 
						"<html><ul>" +
						"<li> Control-(Down or Right)/(Up or Left) = Load Next/Previous File </li>" +
						"<li> Up/Down/Right/Left (when map is focused) = Pan North/South/East/West </li>" +
						"<li> Page Up/Down = Zoom In/Out </li>" +
						"<li> Control-L = Show Data Selector </li>" +
						"<li> Escape = Close/Hide Current Window </li>" +
						"<li> Control-Z = Screen Capture of Current Map </li>" +
						"<li> Control-S = Save Project File (current zoom, dataset source, overlays) </li>" +
						"<li> Control-C = Copy Current Map Image to Clipboard </li>" +
						"</ul></html>";
				JOptionPane.showMessageDialog(finalThis, str, "Keyboard Shortcuts", JOptionPane.INFORMATION_MESSAGE);
			}
		});
        choicePanel.add(shortcutLink, "br");
        choicePanel.add(new JLabel("<html><i>Control-(Down or Right)/(Up or Left) = Load Next/Previous File</i></html>"), "center");
...
//                    System.out.println("tab page change event -- SINGLE FILE");
                    ...
                    resultsList.setFilters(new FilterPipeline(new PatternFilter(".*", 0, 0)));		            
                    resultsList.setModel(listModel);
...
//                    jlInstructions.setText("Hold the 'Shift' or 'Control' keys to make multiple selections");
                    ...
                    if (! dataSourcePanel.getDataType().equals(WCTDataSourceDB.LOCAL_DISK)) {
                        instructionPanel.add(cacheIconPanel, BorderLayout.WEST, 0);
                    }
                    else {
                        instructionPanel.remove(cacheIconPanel);
                    }
...
//        this.getRootPane().setDefaultButton(null);
    ...
    protected void createAutoReloadTimer(long duration) {
...
//                	lockInterface(true);
				...
				String s = df.format(duration - clockTime);
				// 0123456789
				// 00:00.000				
				reloadTimerLabel.setText(StringUtils.rightPad(s, 9, "0").substring(0, 7));
				viewer.getStatusBar().setProgressText("Auto-Reload: "+StringUtils.rightPad(s, 9, "0").substring(0, 7));
			}
		});
    	reloadTimer.start();
    ...
    private void lockInterface(boolean isLocked) {

    	if (isLocked) {
    		DisabledPanel.setEnabledRecursive(this, ! isLocked, new JComponent[] {reloadLabel, reloadTimerLabel, jcomboAutoReload});
    	}
    	else {
    		DisabledPanel.setEnabledRecursive(this, ! isLocked, new JComponent[] {});
    	}
    	
//    	if (isLocked) 
//    		DisabledPanel.disable(this);
//    	else 
//    		DisabledPanel.enable(this);
//
//
//    	
//    	
//    	WCTUiUtils.setPanelEnabled(bottomPanel, ! isLocked);
//    	WCTUiUtils.setPanelEnabled(threddsPanel, ! isLocked);
//    	WCTUiUtils.setPanelEnabled(dataSourcePanel, ! isLocked);
//    	dataSourcePanel.setIsLoading(isLocked);
//    	
    	
  	
    	reloadLabel.setEnabled(true);
    	reloadTimerLabel.setEnabled(true);
    	jcomboAutoReload.setEnabled(true);
    }
	...
	private void addFavorite() {
        if (getDataSourcePanel().getDataType().equals(WCTDataSourceDB.THREDDS)) {
        	...
        	for (URL url : urls) {
        		String name = null;
        		String[] pathDirs = url.toString().split("/");
        		if (pathDirs[pathDirs.length-1].equalsIgnoreCase("Best")) {
        			name = pathDirs[pathDirs.length-2] + "/" + pathDirs[pathDirs.length-1] + " (TDS: "+url.getHost()+")";	
        		}
        		else {
        			name = pathDirs[pathDirs.length-1] + " (TDS: "+url.getHost()+")";
        		}
...
//                	dd.process();
                ...
                // Check for use of auto-reload timer.  If we are auto-reloading, then always remove file from cache
                boolean isAutoReload = ! jcomboAutoReload.getSelectedItem().toString().contains("Off");
				if (isAutoReload) {
            		try {
            			viewer.getGridDatasetRaster().close();
            			GridDatasetUtils.closeCached();
            			WCTTransfer.removeFromCache(url);
            		} catch (Exception e) {
            			e.printStackTrace();
                ...
                if (backgroundThread) {
                    ...
                    foxtrot.Worker.post(new foxtrot.Task() {
                        public Object run() {
                            ...
                            try {
                            	viewer.loadFile(url, true, false, isAutoReload, jcbExtent.isSelected());
...
//        }
    ...
    public void setIsLoading(boolean isLoading) {
    	...
    	if (! jcomboAutoReload.getSelectedItem().toString().contains("Off")) {
    		isLoading = true;
    		// keep it true and buttons disabled if we are on autoload
    	}
...
//        System.out.println("done.");
    ...
    public void selectNextURL() {
    	if (resultsList.getSelectedIndex() == resultsList.getElementCount()-1) {
    		resultsList.setSelectedIndex(0);
    	}
    	else {
    ...
    public void selectPreviousURL() {
    	if (resultsList.getSelectedIndex() == 0) {
    		resultsList.setSelectedIndex(resultsList.getElementCount()-1);
    	}
    	else {
    		resultsList.setSelectedIndex(resultsList.getSelectedIndex()-1);
    	}
    ...
    final public class KeyHandler implements KeyListener {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
                dispose();
                viewer.getMapPane().requestFocusInWindow();
            }
