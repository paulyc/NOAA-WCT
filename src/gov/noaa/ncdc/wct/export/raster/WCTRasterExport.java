...
//        }
    ...
    throws WCTExportNoDataException, IOException, InvalidRangeException {
        ...
        NetcdfFileWriter ncfile = NetcdfFileWriter.createNew(Version.netcdf3, f.toString());
        
//        FileWriter2 writer2 = new FileWriter2(ncIn, f.toString(), Version.netcdf3, null); 
//        NetcdfFile ncOut = writer2.write(new CancelTask() {
//			@Override
...
//		});
        ...
        // define dimensions
        ucar.nc2.Dimension latDim = ncfile.addDimension(null, "lat", raster.getHeight());
        ucar.nc2.Dimension lonDim = ncfile.addDimension(null, "lon", raster.getWidth());
        ucar.nc2.Dimension timeDim = null;
        ucar.nc2.Dimension[] dimArray;
        if (isTimeDimensionNeeded) {
            timeDim = ncfile.addDimension(null, "time", 1);
...
//        System.out.println("exporting variable '"+wctRaster.getVariableName()+"' to CF-Gridded NetCDF");
        ncfile.addVariable(null, wctRaster.getVariableName(), ucar.ma2.DataType.DOUBLE, Arrays.asList(dimArray));
        ncfile.addVariableAttribute(ncfile.findVariable(wctRaster.getVariableName()), new Attribute("long_name", wctRaster.getLongName()));
        ncfile.addVariableAttribute(ncfile.findVariable(wctRaster.getVariableName()), new Attribute("missing_value", wctRaster.getNoDataValue()));
        ncfile.addVariableAttribute(ncfile.findVariable(wctRaster.getVariableName()), new Attribute("units", wctRaster.getUnits()));
        ...
        ncfile.addGroupAttribute(null, new Attribute("title", title));
        ncfile.addGroupAttribute(null, new Attribute("Conventions", "CF-1.0"));
        ncfile.addGroupAttribute(null, new Attribute("History", "Exported to NetCDF-3 CF-1.0 conventions by the NOAA Weather and " +
        ...
        ncfile.addVariable(null, "lat", ucar.ma2.DataType.DOUBLE, Arrays.asList(new ucar.nc2.Dimension[] {latDim}));
        ncfile.addVariableAttribute(ncfile.findVariable("lat"), new Attribute("units", "degrees_north"));
        ncfile.addVariableAttribute(ncfile.findVariable("lat"), new Attribute("spacing", String.valueOf(cellHeight)));
        ncfile.addVariableAttribute(ncfile.findVariable("lat"), new Attribute("datum", "NAD83 - NOAA Standard"));
        // float lon(lon) ;
        // lon:units = "degrees_east" ;
        ncfile.addVariable(null, "lon", ucar.ma2.DataType.DOUBLE, Arrays.asList(new ucar.nc2.Dimension[] {lonDim}));
        ncfile.addVariableAttribute(ncfile.findVariable("lon"), new Attribute("units", "degrees_east"));         
        ncfile.addVariableAttribute(ncfile.findVariable("lon"), new Attribute("spacing", String.valueOf(cellWidth)));
        ncfile.addVariableAttribute(ncfile.findVariable("lon"), new Attribute("datum", "NAD83 - NOAA Standard"));

        if (isTimeDimensionNeeded) {
            // int time(time) ;
            // time:units = "hours" ;
            ncfile.addVariable(null, "time", ucar.ma2.DataType.INT, Arrays.asList(new ucar.nc2.Dimension[] {timeDim}));
            ncfile.addVariableAttribute(ncfile.findVariable("time"), new Attribute("units", "seconds since 1970-1-1"));     
        }
        
        ncfile.addGroupAttribute(null, new Attribute("geographic_datum_ESRI_PRJ", WCTProjections.NAD83_ESRI_PRJ));
        ncfile.addGroupAttribute(null, new Attribute("geographic_datum_OGC_WKT", WCTProjections.NAD83_WKT));
        ...
        ncfile.write(ncfile.findVariable(wctRaster.getVariableName()), valueArray);
        ...
        // Bounds is set the coverage extent of the grid cells, netcdf conventions require
        // that the lat/lon coordinate arrays are defined using the centroids of the grid
        // cells, so we must account for that here by adding half a grid cell to determine the
        // centroid coordinates.
        for (int n=0; n<raster.getHeight(); n++) {
//        	System.out.println((bounds.y + cellHeight/2.0 + n*cellHeight) + " " +cellWidth+" , "+bounds);
            latArray.setDouble(latIndex.set(n), bounds.y + cellHeight/2.0 + n*cellHeight);
        }
        for (int n=0; n<raster.getWidth(); n++) {
//        	System.out.println((bounds.x + cellWidth/2.0 + n*cellWidth) + " " +cellWidth+" , "+bounds);
            lonArray.setDouble(lonIndex.set(n), bounds.x + cellWidth/2.0 + n*cellWidth);
        }
        ncfile.write(ncfile.findVariable("lat"), latArray);
        ncfile.write(ncfile.findVariable("lon"), lonArray);
        ...
        if (isTimeDimensionNeeded) {
            ...
            ncfile.write(ncfile.findVariable("time"), timeArray);
        }
