...
implements DataDecodeListener, GeneralProgressListener {
    ...
    private double[] radialExportCappiHeightsInMeters = null;
        	...
        	if (outputFormat == ExportFormat.NATIVE) {
        		...
        		if (scannedFile.getLastScanResult().getDataType() == SupportedDataType.RADIAL &&
        			...
        			if (saveName.endsWith(ext)) {
            			...
            			file = new File(file.toString() + File.separator + saveName + ext);
        			}
        		}
        	}
        }
...
//                writeListeners.add(progressListener);
                ...
                FileWriter2 writer2 = new FileWriter2(ncIn, fileString, Version.netcdf3, null); 
...
//                   System.out.println("CLOSED INPUT FILE.  ");
    	...
    	// Data Export is not supported for Image (png/jpg/gif) formats
    	if (currentDataType == SupportedDataType.IMAGEFILE) {
    		throw new WCTExportException("The WCT does not support the data export of image files.  Use the 'Native Format' output format to copy the file to a local directory.");
    	}
    	if (currentDataType == SupportedDataType.TEXTFILE) {
    		throw new WCTExportException("The WCT does not support the data export of text files.  Use the 'Native Format' output format to copy the file to a local directory.");
    	}
...
//                streamingExportProcess.removeGeneralProgressListener(this);                                
            ...
            if (bounds != null && exportGridCellSize > 0.0) {
                // calculate the raster size needed in decimal degrees
                // find number grid cells needed for 'long' side of grid
                double longSide = ( bounds.getWidth() > bounds.getHeight() ) ? bounds.getWidth() : bounds.getHeight();
                this.exportGridSize = (int) (longSide/exportGridCellSize);
...
//                            throw new WCTExportException("For lat/lon datasets, currently only Gridded NetCDF export is supported");
                            ...
                            System.out.println("NATIVE GRID BOUNDS: "+nativeGrid.getBounds());
                        }
                    }
                    else {
...
//                        rasterizer.setSize(new Dimension(exportGridSize, exportGridSize));
                    ...
                    if (radialExportCappiHeightsInMeters == null) {
                    	radialDatasetRaster.process(dataURL.toString(), bounds);
                    }
                    else {
                    	radialDatasetRaster.processCAPPI(dataURL.toString(), bounds, 
                    			radialExportCappiHeightsInMeters, radialExportCappiInterpolationType);
                    }
                    ...
                    if (outputFormat == ExportFormat.GEOTIFF_GRAYSCALE_8BIT) {
                    	...
                    	if (radialExportCappiHeightsInMeters == null) {
                			...
                			// Add Radar site altitude to cappi heights, so heights are ASL (above sea level)
                    		double[] cappiHeightsInMeters = new double[radialExportCappiHeightsInMeters.length];
                			for (int n=0; n<radialExportCappiHeightsInMeters.length; n++) {
                				cappiHeightsInMeters[n] = radialExportCappiHeightsInMeters[n] + radialDatasetHeader.getAlt()/3.28083989501312;
                			}
                    		
                    		rasterExport.saveNetCDF(file, genericRaster, cappiHeightsInMeters);
                    	}
                    }
                    else if (outputFormat == ExportFormat.WCT_RASTER_OBJECT_ONLY) {
                    	...
                    	if (this.exportKmlOptions == null) {
                            ...
                            URL paletteURL = ResourceUtils.getInstance().getJarResource(
                    				...
                    				"/config/colormaps/"+NexradSampleDimensionFactory.getDefaultPaletteName(
                    						radialDatasetHeader.getProductCode(), radialDatasetHeader.getVersion()), null);
                ...
                if (outputFormat == ExportFormat.GEOTIFF_GRAYSCALE_8BIT) {
                	...
                	if (this.exportKmlOptions == null) {
                        ...
                        URL paletteURL = ResourceUtils.getInstance().getJarResource(
                				new URL(WCTConstants.CONFIG_JAR_URL), ResourceUtils.CONFIG_CACHE_DIR, 
                				"/config/colormaps/"+NexradSampleDimensionFactory.getDefaultPaletteName(
                						level3Header.getProductCode(), level3Header.getVersion()), null);
...
//      }
    ...
    public void setRadialExportCappiHeightsInMeters(
			double[] radialExportCappiHeightsInMeters) {
		this.radialExportCappiHeightsInMeters = radialExportCappiHeightsInMeters;
	}


	public double[] getRadialExportCappiHeightsInMeters() {
		return radialExportCappiHeightsInMeters;
	}
