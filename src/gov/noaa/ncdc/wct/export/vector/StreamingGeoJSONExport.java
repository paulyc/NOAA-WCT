...
//        bw.newLine();
     ...
     *  
     *  7/24/18 - added polygon support submitted by Alex Neises
     *
    ...
    public void addFeature(Feature feature) throws StreamingProcessException {
        try {
            ...
            if (feature.getDefaultGeometry().getGeometryType().contains("Point")) {
            ...
            else if (feature.getDefaultGeometry().getGeometryType().contains("Polygon")) {
                ...
                bw.write("        [");
                bw.newLine();
                for (int n = 0; n < feature.getDefaultGeometry().getCoordinates().length; n++) {
                    Coordinate[] coords = feature.getDefaultGeometry().getCoordinates();
                    if (n != feature.getDefaultGeometry().getCoordinates().length - 1) {
                        bw.write("          [" + coords[n].x + ", " + coords[n].y + "],");
                    }
                    else {
                        bw.write("          [" + coords[n].x + ", " + coords[n].y + "]");
                    }
            ...
            for (int n=0; n<attNum; n++) {
                if (! featureType.getAttributeType(n).isGeometry()) {
                    ...
                    bw.write("        \""+featureType.getAttributeType(n).getName()+"\": ");                    
                    if (featureType.getAttributeType(n).getType().getName().equals("java.lang.String")) {
                    	// quotes if a string
                        bw.write("\""+att+"\"");                    	
                    }
                    else {
                    	// no quotes if 'real' int, float, double numbers
                        bw.write(""+att+"");
                    }
