...
//        WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, viewer.getDataSelector().getSelectedDataType());
        ...
        try {
        	...
        	final SupportedDataType dataTypeOverride = (viewer.getDataSelector().getSelectedDataType() == SupportedDataType.UNKNOWN) 
        			? null : viewer.getDataSelector().getSelectedDataType();
        	
        	
//      	Put in background thread for UI
//        	URL cacheURL = WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, dataTypeOverride, viewer);
            URL cacheURL = (URL) foxtrot.Worker.post(new foxtrot.Job() {
                public Object run() {
                    try {
                        return WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, dataTypeOverride, viewer);
                    } catch (Exception e) {
                        e.printStackTrace();
                        return null;
                    }
...
//        			"Data Load Error", JOptionPane.ERROR_MESSAGE);
			...
			if (fd != null) {
				System.out.println("Closing FeatureDataset at: "+fd.getLocation());
				fd.release();
				fd.close();
			}
		}
...
//        String varSummaryString = "Number of variables selected: " + );
        ...
        final JComboBox<Object> exportFormat = new JComboBox<Object>(exportFormatMap.keySet().toArray());
...
//                }
    ...
    private void processFormatRemover(JComboBox<Object> exportFormat) {
...
//        URL url = WCTDataUtils.scan(dataUrls[0], fileScanner, true, true, viewer.getDataSelector().getSelectedDataType(), viewer);
        ...
        if (dataType == SupportedDataType.RADIAL) {
            ...
            if (( (selectedExportFormat == ExportFormat.SHAPEFILE ||
            		...
            		selectedExportFormat == ExportFormat.GEOJSON) && 
            		selectedExportFormatString.contains("(Point Centroid)")) || selectedExportFormat == ExportFormat.CSV ) {
    ...
    private void createView4Panel() {
        ...
        if (selectedExportFormatType == ExportFormatType.VECTOR) {
            ...
            if (dataType == SupportedDataType.GRIDDED) {
            	...
            	GridDatatype grid = gridProps.getGrids().get(gridProps.getSelectedGridIndex());
            	int midPointIndexX = grid.getCoordinateSystem().getXHorizAxis().getShape(0)/2;
            	int midPointIndexY = grid.getCoordinateSystem().getYHorizAxis().getShape(0)/2;
            	LatLonPoint llp1 = grid.getCoordinateSystem().getLatLon(midPointIndexX, midPointIndexY);
            	LatLonPoint llp2 = grid.getCoordinateSystem().getLatLon(midPointIndexX+1, midPointIndexY+1);
            	double resolutionX = Math.abs(llp1.getLongitude()-llp2.getLongitude());
            	double resolutionY = Math.abs(llp1.getLatitude()-llp2.getLatitude());
            	String projNote = grid.getCoordinateSystem().getProjection().isLatLon() ? "" : "(at center)";
            	System.out.println(midPointIndexX+" , "+midPointIndexY+" == "+resolutionX + " , "+resolutionY);
            	...
            	double nativeResolutionX = (maxX-minX)/grid.getCoordinateSystem().getXHorizAxis().getShape(0);
            	...
            	double nativeResolutionY = (maxY-minY)/grid.getCoordinateSystem().getYHorizAxis().getShape(0);
            	
                panel.add("center", new JLabel("<html><u>Input Grid Info:</u></html>"));
            	panel.add("left br", new JLabel("Grid Center X Cell Size: "+ WCTUtils.smartFormat(resolutionX) + " deg "+projNote));
            	panel.add("left br", new JLabel("Grid Center Y Cell Size: "+ WCTUtils.smartFormat(resolutionY) + " deg "+projNote));
            	panel.add("left br", new JLabel("Native X Cell Size: "+ WCTUtils.smartFormat(nativeResolutionX) + " "+grid.getCoordinateSystem().getXHorizAxis().getUnitsString()));
            	panel.add("left br", new JLabel("Native Y Cell Size: "+ WCTUtils.smartFormat(nativeResolutionY) + " "+grid.getCoordinateSystem().getYHorizAxis().getUnitsString()));

            }
            
            

            panel.add("p center", new JLabel("<html><u>Output Raster Grid Options:</u></html>"));
...
//              ));
            ...
            if (isLatLon && isRegularSpatial) {
                try {
                    ...
                    panel.add("left br", new JLabel("Cell Size: "+ WCTUtils.smartFormat(yCellSize) + " Degrees"));
                } catch (Exception e) {
...
//                cardLayout.next(mainPanel);
    ...
    private void doExport() {
        ...
        if (dataType == SupportedDataType.RADIAL) {
            ...
            exporter.setRadialExportCappiHeightsInMeters(
            		Double.isNaN(radialPropsPanel.getCappiAltitude()) 
            			? null : new double[] { radialPropsPanel.getCappiAltitude() });
        }
        else if (dataType == SupportedDataType.NEXRAD_LEVEL3 || dataType == SupportedDataType.NEXRAD_LEVEL3_NWS) {
