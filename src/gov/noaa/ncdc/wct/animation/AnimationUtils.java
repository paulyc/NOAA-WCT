    ...
    throws ParseException, NumberFormatException, XPathExpressionException, SAXException, IOException, 
        ...
        if (viewer.getGridProps() != null && (
            ...
            for (int i=0; i<urlsToLoad.length; i++) {
                ...
                if (isTimeMorphing) {
                	doMorphing(urlsToLoad, viewer, morpher, morphSettings, individualProgressListener, operationLog, i);
                }
                else {
...
//                		System.out.println("framehandler.processFrame()");
    ...
    private static BufferedImage[] doMorphing(URL[] urlsToLoad, WCTViewer viewer, 
    		WCTMorphOperation morpher, MorphSettings morphSettings,
    		GeneralProgressListener individualProgressListener, StringBuilder operationLog, int index) throws MalformedURLException {
...
//    			/pub/SL.us008001/DF.of/DC.radar/DS.58sti/SI.kfcx/sn.last
    		...
    		try {
            	...
            	images = processTimeMorphing(viewer, morpher, individualProgressListener, 
            			dataURL, stiURL, index, morphSettings, (index == urlsToLoad.length-2), operationLog);
    		...
    		try {
            	...
            	images = processTimeMorphing(viewer, morpher, individualProgressListener, 
            			urlsToLoad[index], urlsToLoad[index+1], index, morphSettings, (index == urlsToLoad.length-2), operationLog);
			...
			public void processGridCoverage(GridCoverage gc, Date date, int step, long durationInMillis) {
				...
				if (viewer.getFileScanner().getLastScanResult().getDataType() == SupportedDataType.RADIAL ||
					...
					if (date != null) {
						...
						legendProducer.setDateTimeInfo(new String[] { dtinfo });
					}
					try {
						viewer.getLargeLegendPanel().setLegendImage(legendProducer);
					} catch (Exception e1) {
					...
					gc = WCTGridCoverageSupport.applyAlpha2(gc, gc.getSampleDimensions(), viewer.getRadarTransparency());
...
//				}
	...
	public static int frameCount(WCTViewer viewer, URL[] urlsToLoad) 
