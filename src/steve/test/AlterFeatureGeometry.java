...
//        new AlterFeatureGeometry().processStates();
    	...
    	File inputShpfile = new File("C:/work/cag/leaflet-countries.shp");
...
//    	File inputShpfile = new File("D:\\work\\naturalearth\\final\\countries-usa.shp");
    	File outdir = new File("C:\\work\\cag\\");
//    	new AlterFeatureGeometry().append360(inputShpfile, outdir);
    	new AlterFeatureGeometry().append360LeftAndRight(inputShpfile, outdir);
    }
...
//                System.out.println(f.getFeatureType());
    ...
    public void append360LeftAndRight(File inputShpfile, File outdir) {
        ...
        try {
            ...
            StreamingShapefileExport shpExport = new StreamingShapefileExport(new File(outdir.toString()+File.separator+basename+"-360WestAndEast.shp"));            
            FeatureIterator iter = fc.features();
            int geoIndex = 0;
            while (iter.hasNext()) {
                Feature f = iter.next();
...
//                System.out.println(f.getFeatureType());
                ...
                Feature f1 = f.getFeatureType().create(f.getAttributes(null), String.valueOf(geoIndex++));
                System.out.println("native: "+f1.toString().substring(0, 80));
                shpExport.addFeature(f1);
            	...
            	GeometryFactory geoFactory = new GeometryFactory();
                {
                	...
                	Feature f2 = f.getFeatureType().create(f.getAttributes(null), String.valueOf(geoIndex++));
                	System.out.println("-360: "+f2.toString().substring(0, 80));
                	shpExport.addFeature(f2);
                }

                {
                	...
                	Coordinate[] coords = geom.getCoordinates();
                	for (Coordinate c : coords) {
                		c.x += 720;  // add 720 to account for 360 already subtracted
                	}
            ...
            System.out.println("WROTE TO: "+new File(outdir.toString()+File.separator+basename+"-360WestAndEast.shp"));
        } catch (Exception e) {
            e.printStackTrace();
        }
