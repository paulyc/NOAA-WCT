...
public class WCTStatusBar extends JComponent implements MouseMotionListener {
    ...
    private String rangeUnitsAbbreviation = "mi";
    private String zUnitsAbbreviation = "ft";
    ...
    private Point2D pointToReuse = new Point2D.Double();
     ...
     */
    public WCTStatusBar(WCTViewer viewer) {
    	this.viewer = viewer;

    	try {
    		this.rangeUnits = WCTProperties.getWCTProperty("default_range_units") == null ? null : WCTProperties.getWCTProperty("default_range_units");
    		this.rangeUnitsAbbreviation = WCTProperties.getWCTProperty("default_range_units_abbr") == null ? "mi" : WCTProperties.getWCTProperty("default_range_units_abbr");
    		this.zUnits = WCTProperties.getWCTProperty("default_z_units") == null ? null : WCTProperties.getWCTProperty("default_z_units");
    		this.zUnitsAbbreviation = WCTProperties.getWCTProperty("default_z_units_abbr") == null ? "ft" : WCTProperties.getWCTProperty("default_z_units_abbr");			
		} catch (Exception e) {
     ...
     */
    public WCTStatusBar(final Component mapPane, final WCTViewer viewer) {
        this(viewer);
    ...
    /**
     * Grid coord system is used to get lat/lon info.  No units will be used.
     * @param gridCoordSys
     */
    public void setGridInfo(GridCoordSystem gridCoordSystem) {
    	this.gridCoords = gridCoordSystem;
    	this.unitsString = "";
    }
    
    /**
     * Grid coord system is used to get lat/lon info.  Raster is used to get units.
     * @param gridCoordSys
     * @param wctRaster
     */
    public void setGridInfo(GridCoordSystem gridCoordSystem, WCTRaster wctRaster) {
    	this.gridCoords = gridCoordSystem;
    	this.unitsString = wctRaster.getUnits();
    }
    ...
    public void setNexradHeader(NexradHeader header) {
        ...
        if (nexradHeader != null && nexradHeader.getLat() != NexradHeader.NO_SITE_DEFINED) {
            try {
                ...
                this.unitsString = NexradUtilities.getUnits(nexradHeader);
...
//                    progressBar.setString(fmt3.format(height)+" "+fmt3.format(height2)+" "+fmt3.format(height3));
    ...
    public void mouseMoved(final MouseEvent event) {
        if (event instanceof GeoMouseEvent) {
            ...
            if (! viewer.getStationPointFeatures().isEmpty()) {
//            	System.out.println( ((GeoMouseEvent) event).getMapCoordinate(pointToReuse)  );
            	((GeoMouseEvent) event).getMapCoordinate(pointToReuse);
            	FeatureIterator iter = viewer.getStationPointFeatures().features();
            	while (iter.hasNext()) {
            		Feature f = iter.next();
//            		System.out.println(f.getDefaultGeometry().getGeometryType());
            		if (f.getDefaultGeometry().getGeometryType().contains("Polygon")) {
            			if (f.getDefaultGeometry().contains(geofactory.createPoint(new Coordinate(pointToReuse.getX(), pointToReuse.getY())))) {
//            				System.out.println(f.getAttribute(1));
            				value = f.getAttribute(2).toString() + " = " + fmt3.format(Double.parseDouble(f.getAttribute(1).toString()));
            			}
            		}
            	}
            }
            ...
            if (sd != null && ! value.equals("No Data")) {
            	...
            	if (catName.startsWith("Unique:")) {
            		if (nexradHeader != null && 
            				(nexradHeader.getProductType() == NexradHeader.L3RASTER || nexradHeader.getProductType() == NexradHeader.L3RADIAL)) {
            		
            			String s = viewer.getLevel3Header().getDataThresholdString(Integer.parseInt(value));
                		setCoordinate(text[0]+" "+text[1]+"  ("+s+")");
            		}
            		else {
...
//                	setCoordinate(format.format((GeoMouseEvent) event));
           		...
           		if ((! value.equals("No Data")) && viewer.getCurrentDataType() == CurrentDataType.RADAR && 
        				(viewer.getCurrentNexradType() == NexradHeader.L3RASTER || viewer.getCurrentNexradType() == NexradHeader.L3RADIAL)) {
        		
        			String s = ((DecodeL3Header)nexradHeader).getDataThresholdString(Math.round(Float.parseFloat(value)));
            		setCoordinate(text[0]+" "+text[1]+"  ("+s+")");
        		}
        		else {
        			setCoordinate(text[0]+" "+text[1]+"  ("+value+")");
            ...
            if (source.getText().equals("Coordinate Format")) {
                ...
                try {
                    ...
                    if (rangeUnitsChooser.showDialog(WCTStatusBar.this, "Range Units Chooser")) {
                        ...
                        try {
							WCTProperties.setWCTProperty("default_range_units", rangeUnitsChooser.getUnits());
							WCTProperties.setWCTProperty("default_range_units_abbr", rangeUnitsChooser.getUnitsAbbreviation());
						} catch (Exception e) {
							e.printStackTrace();
						}
                    }
                ...
                try {
                    ...
                    if (zUnitsChooser.showDialog(WCTStatusBar.this, "Height (Z) Units Chooser")) {
                        ...
                        try {
							WCTProperties.setWCTProperty("default_z_units", zUnitsChooser.getUnits());
							WCTProperties.setWCTProperty("default_z_units_abbr", zUnitsChooser.getUnitsAbbreviation());
						} catch (Exception e) {
							e.printStackTrace();
						}
                    }
