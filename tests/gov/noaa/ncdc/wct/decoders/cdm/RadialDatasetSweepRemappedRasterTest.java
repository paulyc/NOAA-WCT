...
public class RadialDatasetSweepRemappedRasterTest {
    ...
    public void testRange() {
        ...
        ProjectionPointImpl projPoint = (ProjectionPointImpl)trans.latLonToProj(lat2, lon2);
        
        
//        double distance = projPoint.distance(trans.getOriginLon(), trans.getOriginLat());
        // above is broken in 4.5 NCJ update
        double distance = -999;
        ...
        double azimuth = Math.toDegrees(Math.atan(projPoint.getX()/projPoint.getY())); // reverse for meteorlogical coord system
        if (projPoint.getY() < 0) {
            azimuth += 180;
        }
        if (projPoint.getY() >= 0 && projPoint.getX() <= 0) {
            azimuth += 360;
        }
